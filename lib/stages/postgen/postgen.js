  ...
  for (const comp of bomJson.components) {
    ...
    if (comp.properties) {
      for (const aprop of comp.properties) {
        if (aprop.name === "SrcFile" && aprop.value && comp.type !== "file") {
          if (Array.isArray(aprop.value)) {
            for (const chain of aprop.value) {
              if (Array.isArray(chain) && chain.length > 0) {
                bomSrcFiles.add(chain[0]);
              }
            }
          } else if (typeof aprop.value === "string") {
            bomSrcFiles.add(aprop.value);
          }
        }
      }
    }
