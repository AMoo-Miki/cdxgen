...
function extractProperty(obj, propName) {
  ...
  // cdxgen permits some properties (e.g. SrcFile dependency chains) to carry
  // an Array<Array<string>> value. JSONata helpers used in audit rules
  // (e.g. $contains, $startsWith) only operate on strings, so flatten array
  // values to a stable string representation that preserves substrings the
  // rules look for (such as ".asar#/").
  if (Array.isArray(prop.value)) {
    const flat = prop.value
      .flat(Infinity)
      .filter((v) => v != null)
