...
`,
  ...
  describe("node_modules multi-ecosystem filtering", () => {
    ...
    it("merges SrcFile chains for packages that appear in multiple package-lock.json files", async () => {
      const projectPath = join(
        ...
        "test",
        "data",
        "multi-source-npm-project",
      );
      const bomData = await createNodejsBom(projectPath, {
        installDeps: false,
        multiProject: true,
        projectType: ["js"],
        specVersion: 1.7,
      ...
      const getSrcFile = (name) =>
        (byName[name]?.properties || []).filter((p) => p.name === "SrcFile");
      for (const c of components) {
        if (c.type === "application") continue;
        assert.deepStrictEqual(
          getSrcFile(c.name).length,
          1,
          `${c.name} should have exactly one SrcFile property`,
        );
      }
      assert.deepStrictEqual(getSrcFile("feature-a")[0].value, [
        ["packages/web/package-lock.json"],
      ]);
      assert.deepStrictEqual(getSrcFile("feature-b")[0].value, [
        ["packages/worker/package-lock.json"],
      ]);
      const sharedCore = getSrcFile("shared-core")[0];
      const sharedFirstSegs = new Set(sharedCore.value.map((c) => c[0]));
      assert.ok(sharedFirstSegs.has("packages/web/package-lock.json"));
      assert.ok(sharedFirstSegs.has("packages/worker/package-lock.json"));
      const sharedSegs = new Set(sharedCore.value.flat());
      assert.ok(sharedSegs.has("pkg:npm/feature-a@1.2.0"));
      assert.ok(sharedSegs.has("pkg:npm/feature-b@1.3.0"));
      const lowLevel = getSrcFile("low-level-util")[0];
      for (const chain of lowLevel.value) {
        assert.deepStrictEqual(
          chain[chain.length - 1],
          "pkg:npm/shared-core@2.5.1",
        );
      }
    });

    it("merges SrcFile chains for packages that appear in multiple Gemfile.lock files", async () => {
      const projectPath = join(
        dirname(fileURLToPath(import.meta.url)),
        "..",
        "..",
        "test",
        "data",
        "multi-source-ruby-project",
      );
      const bomData = await createRubyBom(projectPath, {
        installDeps: false,
        multiProject: true,
        projectType: ["ruby"],
        specVersion: 1.7,
      ...
      for (const c of components) {
        if (c.type === "application") continue;
        assert.deepStrictEqual(getSrcFile(c.name).length, 1);
      }
      const sharedCore = getSrcFile("shared-core")[0];
      const firstSegs = new Set(sharedCore.value.map((c) => c[0]));
      assert.ok(firstSegs.has("services/web/Gemfile.lock"));
      assert.ok(firstSegs.has("services/worker/Gemfile.lock"));
      const allSegs = new Set(sharedCore.value.flat());
      assert.ok(allSegs.has("pkg:gem/feature-a@1.2.0"));
      assert.ok(allSegs.has("pkg:gem/feature-b@1.3.0"));
      const lowLevel = getSrcFile("low-level-util")[0];
      for (const chain of lowLevel.value) {
        assert.deepStrictEqual(
          chain[chain.length - 1],
          "pkg:gem/shared-core@2.5.1",
        );
      }
    });

    it("merges SrcFile chains for packages that appear in multiple yarn.lock files", async () => {
      const projectPath = join(
        dirname(fileURLToPath(import.meta.url)),
        "..",
        "..",
      ...
      const sharedCore = getSrcFile("shared-core")[0];
      const firstSegs = new Set(sharedCore.value.map((c) => c[0]));
      assert.ok(firstSegs.has("packages/web/yarn.lock"));
      assert.ok(firstSegs.has("packages/worker/yarn.lock"));
      const allSegs = new Set(sharedCore.value.flat());
      assert.ok(allSegs.has("pkg:npm/feature-a@1.2.0"));
      assert.ok(allSegs.has("pkg:npm/feature-b@1.3.0"));
    });

    it("merges SrcFile chains for packages that appear in multiple pnpm-lock.yaml files", async () => {
      const projectPath = join(
        dirname(fileURLToPath(import.meta.url)),
        "..",
        "..",
      ...
      const sharedCore = getSrcFile("shared-core")[0];
      const firstSegs = new Set(sharedCore.value.map((c) => c[0]));
      assert.ok(firstSegs.has("packages/web/pnpm-lock.yaml"));
      assert.ok(firstSegs.has("packages/worker/pnpm-lock.yaml"));
      const allSegs = new Set(sharedCore.value.flat());
      assert.ok(allSegs.has("pkg:npm/feature-a@1.2.0"));
      assert.ok(allSegs.has("pkg:npm/feature-b@1.3.0"));
    });

    it("merges SrcFile chains for packages that appear in multiple poetry.lock files", async () => {
      const projectPath = join(
        dirname(fileURLToPath(import.meta.url)),
        "..",
        "..",
        "test",
        "data",
        "multi-source-python-project",
      );
      const bomData = await createPythonBom(projectPath, {
        installDeps: false,
        multiProject: true,
        projectType: ["python"],
        specVersion: 1.7,
      ...
      const sharedCore = getSrcFile("shared-core")[0];
      const firstSegs = new Set(sharedCore.value.map((c) => c[0]));
      assert.ok(firstSegs.has("services/web/poetry.lock"));
      assert.ok(firstSegs.has("services/worker/poetry.lock"));
      const allSegs = new Set(sharedCore.value.flat());
      assert.ok(allSegs.has("pkg:pypi/feature-a@1.2.0"));
      assert.ok(allSegs.has("pkg:pypi/feature-b@1.3.0"));
      const lowLevel = getSrcFile("low-level-util")[0];
      for (const chain of lowLevel.value) {
        assert.deepStrictEqual(
          chain[chain.length - 1],
          "pkg:pypi/shared-core@2.5.1",
        );
      }
    });

    it("merges SrcFile chains for packages that appear in multiple composer.lock files", () => {
      const projectPath = join(
        dirname(fileURLToPath(import.meta.url)),
        "..",
        "..",
        "test",
        "data",
        "multi-source-composer-project",
      );
      const bomData = createPHPBom(projectPath, {
        installDeps: false,
        multiProject: true,
        projectType: ["php"],
        specVersion: 1.7,
      ...
      const components = bomData?.bomJson?.components || [];
      const byName = Object.fromEntries(components.map((c) => [c.name, c]));
      const getSrcFileProps = (comp) =>
        (comp?.properties || []).filter((p) => p.name === "SrcFile");
      // Every package must end up with EXACTLY ONE SrcFile property, even
      // when found across multiple manifests — the chains merge, not the
      // property entries.
      for (const c of components) {
        if (c.type === "application") continue;
        const sfProps = getSrcFileProps(c);
        assert.deepStrictEqual(
          sfProps.length,
          1,
          `${c.name} should have exactly 1 SrcFile property (got ${sfProps.length})`,
        );
      }
      // feature-a only lives under web/, feature-b only under worker/.
      const featureA = getSrcFileProps(byName["feature-a"])[0];
      assert.deepStrictEqual(featureA.value, [
        ["services/web/composer.lock"],
      ]);
      const featureB = getSrcFileProps(byName["feature-b"])[0];
      assert.deepStrictEqual(featureB.value, [
        ["services/worker/composer.lock"],
      ]);
      // shared-core: reached via BOTH services. The merged SrcFile must
      // contain a chain whose first element is web/composer.lock AND a
      // chain whose first element is worker/composer.lock.
      const sharedCore = getSrcFileProps(byName["shared-core"])[0];
      assert.ok(sharedCore, "shared-core must have SrcFile");
      assert.ok(Array.isArray(sharedCore.value));
      assert.ok(Array.isArray(sharedCore.value[0]));
      const sharedFirstSegs = new Set(sharedCore.value.map((c) => c[0]));
      assert.ok(
        sharedFirstSegs.has("services/web/composer.lock"),
        "shared-core should be traceable to web composer.lock",
      );
      assert.ok(
        sharedFirstSegs.has("services/worker/composer.lock"),
        "shared-core should be traceable to worker composer.lock",
      );
      // shared-core ancestry must include feature-a (via web) AND
      // feature-b (via worker).
      const sharedSegs = new Set(sharedCore.value.flat());
      assert.ok(sharedSegs.has("pkg:composer/acme/feature-a@1.2.0"));
      assert.ok(sharedSegs.has("pkg:composer/acme/feature-b@1.3.0"));
      // low-level-util: deepest leaf, must inherit chains from both services
      // with shared-core as the tail of every chain.
      const lowLevel = getSrcFileProps(byName["low-level-util"])[0];
      assert.ok(Array.isArray(lowLevel.value));
      const lowLevelFirstSegs = new Set(lowLevel.value.map((c) => c[0]));
      assert.ok(lowLevelFirstSegs.has("services/web/composer.lock"));
      assert.ok(lowLevelFirstSegs.has("services/worker/composer.lock"));
      for (const chain of lowLevel.value) {
        assert.deepStrictEqual(
          chain[chain.length - 1],
    ...
    it("merges SrcFile chains for packages that appear in multiple project.assets.json files", async () => {
      const projectPath = join(
        dirname(fileURLToPath(import.meta.url)),
        "..",
        "..",
        "test",
        "data",
        "multi-source-csharp-project",
      );
      const bomData = await createCsharpBom(projectPath, {
        installDeps: false,
        multiProject: true,
        projectType: ["csharp"],
        specVersion: 1.7,
      ...
      const sharedCore = getSrcFile("SharedCore")[0];
      const firstSegs = new Set(sharedCore.value.map((c) => c[0]));
      assert.ok(firstSegs.has("src/Web/obj/project.assets.json"));
      assert.ok(firstSegs.has("src/Worker/obj/project.assets.json"));
      const allSegs = new Set(sharedCore.value.flat());
      assert.ok(allSegs.has("pkg:nuget/FeatureA@1.2.0"));
      assert.ok(allSegs.has("pkg:nuget/FeatureB@1.3.0"));
      assert.ok(allSegs.has("pkg:nuget/Acme.Web@1.0.0"));
      assert.ok(allSegs.has("pkg:nuget/Acme.Worker@1.0.0"));
      const lowLevel = getSrcFile("LowLevelUtil")[0];
      for (const chain of lowLevel.value) {
        assert.deepStrictEqual(
          chain[chain.length - 1],
          "pkg:nuget/SharedCore@2.5.1",
        );
      }
    });

    it("merges SrcFile chains for packages that appear in multiple packages.lock.json files", async () => {
      const projectPath = join(
        dirname(fileURLToPath(import.meta.url)),
        "..",
        "..",
      ...
      const sharedCore = getSrcFile("SharedCore")[0];
      const firstSegs = new Set(sharedCore.value.map((c) => c[0]));
      assert.ok(firstSegs.has("src/Web/packages.lock.json"));
      assert.ok(firstSegs.has("src/Worker/packages.lock.json"));
      const allSegs = new Set(sharedCore.value.flat());
      assert.ok(allSegs.has("pkg:nuget/FeatureA@1.2.0"));
      assert.ok(allSegs.has("pkg:nuget/FeatureB@1.3.0"));
    });

    it("merges SrcFile chains for packages that appear in multiple paket.lock files", async () => {
      const projectPath = join(
        dirname(fileURLToPath(import.meta.url)),
        "..",
        "..",
      ...
      const sharedCore = getSrcFile("SharedCore")[0];
      const firstSegs = new Set(sharedCore.value.map((c) => c[0]));
      assert.ok(firstSegs.has("src/Web/paket.lock"));
      assert.ok(firstSegs.has("src/Worker/paket.lock"));
      const allSegs = new Set(sharedCore.value.flat());
      assert.ok(allSegs.has("pkg:nuget/FeatureA@1.2.0"));
      assert.ok(allSegs.has("pkg:nuget/FeatureB@1.3.0"));
    });

    it("merges SrcFile chains for packages that appear in multiple conan.lock files", async () => {
      const projectPath = join(
        dirname(fileURLToPath(import.meta.url)),
        "..",
        "..",
        "test",
        "data",
        "multi-source-conan-project",
      );
      const bomData = await createCppBom(projectPath, {
        installDeps: false,
        multiProject: true,
        projectType: ["conan"],
        specVersion: 1.7,
      ...
      const sharedCore = getSrcFile("shared-core")[0];
      const firstSegs = new Set(sharedCore.value.map((c) => c[0]));
      assert.ok(firstSegs.has("modules/web/conan.lock"));
      assert.ok(firstSegs.has("modules/worker/conan.lock"));
      const allSegs = new Set(sharedCore.value.flat());
      assert.ok(allSegs.has("pkg:conan/feature-a@1.2.0"));
      assert.ok(allSegs.has("pkg:conan/feature-b@1.3.0"));
      const lowLevel = getSrcFile("low-level-util")[0];
      for (const chain of lowLevel.value) {
        assert.deepStrictEqual(
          chain[chain.length - 1],
          "pkg:conan/shared-core@2.5.1",
        );
      }
    });
  ...
  describe("BOM generation with SrcFile tracking", () => {
    it("should include SrcFile property for Go packages from go.sum", async () => {
      ...
      const srcFileProp = result[0].properties.find(p => p.name === "SrcFile");
      ...
      assert.equal(srcFileProp.value[0][0], "/workspace/project/go.sum", "should have correct path");
    });
    ...
    it("should include SrcFile property for Conan packages from conan.lock", async () => {
      ...
      const srcFileProp = result.pkgList[0].properties.find(
        (p) => p.name === "SrcFile",
      );
      assert.ok(srcFileProp, "should have SrcFile property");
      assert.ok(Array.isArray(srcFileProp.value), "SrcFile should be array");
      assert.ok(
        Array.isArray(srcFileProp.value[0]),
        "SrcFile should be array of arrays",
      );
      assert.equal(srcFileProp.value[0][0], "conan.lock");
    });
    ...
    it("should include SrcFile property for Python packages from Pipfile.lock", async () => {
      ...
      assert.ok(Array.isArray(srcFileProp.value), "SrcFile should be array");
      assert.ok(Array.isArray(srcFileProp.value[0]), "SrcFile should be array of arrays");
      assert.equal(srcFileProp.value[0][0], "/workspace/project/Pipfile.lock", "should have correct path");
    });
    ...
    it("should use absolute paths in multi-project scenarios", async () => {
      ...
      const srcFile1 = result1[0].properties.find(p => p.name === "SrcFile");
      const srcFile2 = result2[0].properties.find(p => p.name === "SrcFile");
      
      assert.ok(Array.isArray(srcFile1.value), "SrcFile should be array");
      assert.ok(Array.isArray(srcFile1.value[0]), "SrcFile should be array of arrays");
      assert.equal(srcFile1.value[0][0], "/workspace/project/backend/go.sum");
      assert.equal(srcFile2.value[0][0], "/workspace/project/frontend/go.sum");
      assert.notEqual(
        srcFile1.value[0][0],
        srcFile2.value[0][0],
