...
import { findJSImportsExports, findJavaImports, findRubyImports, findClojureImports } from "../helpers/analyzer.js";
...
export async function createPythonBom(path, options) {
  ...
  return buildBomNSData(options, pkgList, "pypi", {
    allImports,
    src: path,
    filename: metadataFilenames.length ? metadataFilenames.join(", ") : metadataFilename,
    dependencies,
    parentComponent,
    formulationList,
  });
...
export function createCppBom(path, options) {
  ...
  if (conanLockFiles.length) {
    for (const f of conanLockFiles) {
      ...
      } = parseConanLockData(conanLockData, f);
