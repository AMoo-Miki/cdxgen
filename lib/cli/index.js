...
import { findClojureImports, findJavaImports, findRubyImports } from "../helpers/analyzer.js";
import {
...
// Map of git hosting domains to their PURL type names
...
// Pattern to match git hosting URLs in version fields
// Matches formats like:
// - git+https://git@github.com/user/repo.git#v1.0.0
// - https://github.com/user/repo#v1.0.0 (without .git)
// - git+ssh://git@gitlab.com/group/project.git#main
// - ssh://git@bitbucket.org/user/repo.git#develop
const domainPattern = Object.keys(GIT_HOSTING_DOMAINS)
  .map((d) => d.replace(/\./g, "\\."))
  .join("|");
const GIT_HOSTING_URL_PATTERN = new RegExp(
  `^(?:git\\+)?(https?|ssh):\\/\\/(?:git@)?(${domainPattern})\\/(.+?)(?:\\.git)?(?:#(.*))?$`,
  "i",
);
...
function addComponent(
  ...
  if (!isRootPkg) {
    ...
    // Handle GitHub, GitLab, and Bitbucket git URLs in version field
    // Example: "git+https://git@github.com/user/repo.git#v1.0.0"
    // Example: "git+ssh://git@gitlab.com/group/project.git#main"
    let pptype = ptype;
    let pversion = version;
    let pnamespace = group;
    let pname = name;
    const m = version.match(GIT_HOSTING_URL_PATTERN);
    if (m !== null) {
      // m[1] = protocol (https, http, or ssh)
      // m[2] = matched domain (github.com, gitlab.com, or bitbucket.org)
      // m[3] = path (user/repo or group/subgroup/project)
      // m[4] = ref (tag, branch, or commit after #)
      pptype = GIT_HOSTING_DOMAINS[m[2].toLowerCase()]; // Look up platform name from domain
      pversion = m[4] || ""; // Extract the commit/tag after #

      // Extract namespace (user/org) and name (repo) from the path
      // For GitHub/Bitbucket: m[3] = "user/repo"
      // For GitLab: m[3] could be "user/repo" or "group/subgroup/project"
      const pathParts = m[3].split("/");
      if (pathParts.length >= 2) {
        // For GitLab with nested groups (e.g., "group/subgroup/project"),
        // namespace is everything except the last part
        pnamespace = pathParts.slice(0, -1).join("/");
        pname = pathParts[pathParts.length - 1]; // Last part is the repo/project name
      }
    }
...
export async function createJavaBom(path, options) {
      ...
      if (
        ...
        if (f === firstPom) {
          ...
          if (result.status === 0) {
            if (safeExistsSync(tempMvnParentTree)) {
              ...
              const parsedList = parseMavenTree(mvnTreeString, f, path);
        ...
        if (result.status !== 0 || result.error) {
          ...
          if (pomFiles.length === 1 || DEBUG_MODE || PREFER_MAVEN_DEPS_TREE) {
            console.log(`\nError processing Maven project: ${f}`);
            // Check if maven is not installed
            if (
              result.stderr?.includes("command not found") ||
              result.stderr?.includes("mvn: not found") ||
              result.error?.code === "ENOENT"
            ) {
              console.error(
                "\nMaven (mvn) is not installed or not available in PATH.",
              );
              console.error(
                "Maven Wrapper (mvnw) was also not found in this project.",
              );
              console.log(
                "\nOptions to fix this:",
              );
              ...
              console.log(
                "2. Or add Maven Wrapper to your project: mvn wrapper:wrapper",
              );
            } else {
              if (result.stdout) {
          ...
          console.log(
            `\nFalling back to parsing pom.xml file: ${f}. Only direct dependencies would get included!`,
          );
          thoughtLog(
  ...
  if (
    ...
    if (sstdout) {
      ...
      // Find the build file
      const buildFile = safeExistsSync(join(gradleRootPath, "build.gradle")) 
        ? join(gradleRootPath, "build.gradle")
        : safeExistsSync(join(gradleRootPath, "build.gradle.kts"))
  ...
  // Analyze Java source files for import statements to populate occurrences
  if (!hasAnyProjectType(["docker", "oci", "container", "os"], options, false)) {
    if (DEBUG_MODE) {
      console.log(`Analyzing Java source files for import statements at ${path}`);
    }
    try {
      const javaImports = findJavaImports(path);
      if (javaImports && Object.keys(javaImports).length) {
        pkgList = addJavaOccurrences(pkgList, javaImports);
      }
    } catch (_err) {
      if (DEBUG_MODE) {
        console.log('Java import analysis failed:', _err.message);
      }
    }
  }
...
export async function createNodejsBom(path, options) {
  ...
  const npmInstallCount =
    options.npmInstallCount ??
    (process.env.NPM_INSTALL_COUNT !== undefined
      ? Number.parseInt(process.env.NPM_INSTALL_COUNT, 10)
      : -1);
  let anyInstallSuccess = false;
  // Automatic npm install logic.
  // When npmInstallCount is negative, auto-install is not limited by the number of package.json files.
  // When npmInstallCount >= 0, auto-install is only performed if the number of package.json files is within the limit.
  if (
    (pkgJsonLockFiles?.length === 0 ||
      pkgJsonLockFiles?.length < pkgJsonFiles?.length - 1) &&
    yarnLockFiles?.length === 0 &&
    pnpmLockFiles?.length === 0 &&
    (npmInstallCount < 0 || pkgJsonFiles?.length <= npmInstallCount) &&
    options.installDeps
  ) {
  ...
  if (
    ...
    thoughtLog(`**PNPM**: Processing ${pnpmLockFiles.length} pnpm-lock.yaml file(s).`);
    for (const f of pnpmLockFiles) {
      ...
      const parsedList = await parsePnpmLock(
        ...
        workspaceDirectDeps,
        depsWorkspaceRefs,
        path,  // Pass project root for relative path resolution
      );
      const dlist = parsedList.pkgList;
      if (dlist?.length) {
        pkgList = pkgList.concat(dlist);
  ...
  if (
    ...
    thoughtLog(`**NPM**: Processing ${pkgLockFiles.length} package-lock.json file(s).`);
    for (const f of pkgLockFiles) {
      if (DEBUG_MODE) {
        console.log(`Parsing ${f}`);
      }
      // Parse package-lock.json if available
      const parsedList = await parsePkgLock(f, options, path);
      const dlist = parsedList.pkgList;
  ...
  if (
    ...
    if (safeExistsSync(pnpmLock)) {
      const pnpmLockObj = await parsePnpmLock(pnpmLock, null, [], {}, {}, {}, {}, path);
  ...
  if (
    ...
    thoughtLog(`**YARN**: Processing ${yarnLockFiles.length} yarn.lock file(s).`);
...
export async function createPythonBom(path, options) {
  let allImports = {};
  let metadataFilename = "";
  const metadataFilenames = []; // Track all requirements files
  let dependencies = [];
  let pkgList = [];
  let formulationList = [];
  const includedAiInventoryTypes = getIncludedAiInventoryTypes(options);
  ...
  // Run pigar for setup.py files to generate requirements.txt
  // This runs right before gathering requirements files so that any generated
  // requirements.txt files are included in the scan
  // Uses an isolated temporary venv to avoid polluting the user's environment
  const setupFiles = getAllFiles(
    path,
    `${options.multiProject ? "**/" : ""}setup.py`,
    options,
  );
  if (setupFiles?.length) {
    thoughtLog(
      `**PYTHON**: Attempting to extract dependencies from ${setupFiles.length} setup.py file(s) using pigar.`,
    );
    for (const f of setupFiles) {
      const setupDir = dirname(f);
      // Run pigar in the same temporary venv that will be used for pip operations
      const pigarResult = await runPigarInVenv(setupDir, tempDir);
      if (!pigarResult?.success && DEBUG_MODE) {
        console.log(`Pigar extraction failed for ${f}: ${pigarResult?.error}`);
      }
    }
  }
  ...
  if (poetryMode) {
    for (const f of poetryFiles) {
      ...
      let retMap = await parsePyLockData(lockData, f, undefined, path);
  ...
  if (requirementsMode || pipenvMode) {
    if (pipenvMode) {
      ...
      if (safeExistsSync(piplockFile)) {
        ...
        const dlist = await parsePiplockData(lockData, piplockFile, path);
        if (dlist?.length) {
          pkgList = pkgList.concat(dlist);
        }
      } else {
  ...
  // Parse extracted-requirements.txt files generated by pigar
  // These are parsed separately and deduplicated against existing packages
  const extractedFiles = getAllFiles(
    path,
    `${options.multiProject ? "**/" : ""}extracted-requirements.txt`,
    options,
  );
  if (extractedFiles?.length) {
    for (const f of extractedFiles) {
      // Get list of packages already in pkgList
      const existingPkgNames = pkgList.map(pkg => pkg.name);
      const extractedData = await parseReqFile(f, false);
      // Only add packages not already in the list (deduplication)
      const newPkgs = extractedData.filter(pkg => !existingPkgNames.includes(pkg.name));
      if (newPkgs?.length) {
        pkgList = pkgList.concat(newPkgs);
      }
    }
    metadataFilenames.push(...extractedFiles);
  }
...
export async function createGoBom(path, options) {
  ...
  if (useGosum && gosumFiles.length) {
    ...
    for (const f of gosumFiles) {
      ...
      const dlist = await parseGosumData(gosumData, f, path);
      if (dlist?.length) {
        pkgList = pkgList.concat(dlist);
      }
    }
  ...
  if (gopkgLockFiles.length) {
    for (const f of gopkgLockFiles) {
      ...
      const dlist = await parseGopkgData(gopkgData, f);
      if (dlist?.length) {
        pkgList = pkgList.concat(dlist);
      }
    }
...
export async function createRustBom(path, options) {
  ...
  if (cargoLockMode) {
    for (const f of cargoLockFiles) {
      ...
      const dlist = await parseCargoData(f, false, pkgFilesMap, path);
      if (dlist?.length) {
        pkgList = pkgList.concat(dlist);
      }
...
export function createCppBom(path, options) {
  ...
  if (conanLockFiles.length) {
    for (const f of conanLockFiles) {
      ...
      } = parseConanLockData(conanLockData, f, path);
    ...
    for (const f of conanFiles) {
      ...
      const dlist = parseConanData(conanData, f);
      if (dlist?.length) {
        pkgList = pkgList.concat(dlist);
      }
    }
...
export function createClojureBom(path, options) {
  ...
  if (leinFiles.length) {
    ...
    // Add evidence.occurrences from source file analysis
    if (pkgList?.length && !hasAnyProjectType(["oci"], options, false)) {
      const clojureImports = findClojureImports(path);
      if (Object.keys(clojureImports).length > 0) {
        pkgList = addClojureOccurrences(pkgList, clojureImports);
      }
    }
  ...
  if (swiftFiles.length) {
    for (const f of swiftFiles) {
      ...
      if (result.stdout) {
        ...
        const retData = parseSwiftJsonTree(treeData, f, path);
...
export function createPHPBom(path, options) {
  ...
  if (!composerLockMode && composerJsonMode && options.installDeps) {
    ...
    if (versionResult.status !== 0 || versionResult.error) {
      console.error(
        "Composer is not installed or not available in PATH. Skipping composer.lock generation.",
      );
      console.log(
      ...
      // Skip composer install attempts since composer is not available
    } else {
      let composerVersion;
      if (DEBUG_MODE) {
      ...
      for (const f of composerJsonFiles) {
        ...
        if (result.status !== 0 || result.error) {
          console.error(`Error running composer in ${basePath}:`);
          if (result.stdout) {
            console.log(result.stdout);
          }
          if (result.stderr) {
  ...
  if (composerLockFiles.length) {
    ...
    for (const f of composerLockFiles) {
      ...
      const retMap = parseComposerLock(f, rootRequires, path);
      if (retMap.pkgList?.length) {
        pkgList = pkgList.concat(retMap.pkgList);
        pkgList = trimComponents(pkgList);
      }
...
export async function createRubyBom(path, options) {
  ...
  if (gemLockFiles.length) {
    for (const f of gemLockFiles) {
      ...
      const retMap = await parseGemfileLockData(gemLockData, f, path);
      if (retMap.pkgList?.length) {
        pkgList = pkgList.concat(retMap.pkgList);
        pkgList = trimComponents(pkgList);
      }
  ...
  // Analyze Ruby source files for require statements to populate occurrences
  if (!hasAnyProjectType(["docker", "oci", "container", "os"], options, false)) {
    if (DEBUG_MODE) {
      console.log(`Analyzing Ruby source files for require statements at ${path}`);
    }
    try {
      const rubyImports = findRubyImports(path);
      if (rubyImports && Object.keys(rubyImports).length) {
        pkgList = addRubyOccurrences(pkgList, rubyImports);
      }
    } catch (_err) {
      if (DEBUG_MODE) {
        console.log('Ruby import analysis failed:', _err.message);
      }
    }
  }
...
export async function createCsharpBom(path, options) {
  ...
  if (projAssetsFiles.length) {
    ...
    for (const af of projAssetsFiles) {
      ...
      const results = parseCsProjAssetsData(pkgData, af, path);
    ...
    for (const af of pkgLockFiles) {
      ...
      pkgData = readFileSync(af, { encoding: "utf-8" });
      const results = parseCsPkgLockData(pkgData, af, path);
      const deps = results["dependenciesList"];
      const dlist = results["pkgList"];
  ...
  if (paketLockFiles.length) {
    ...
    for (const f of paketLockFiles) {
      ...
      pkgData = readFileSync(f, { encoding: "utf-8" });
      const results = parsePaketLockData(pkgData, f, path);
      const dlist = results.pkgList;
      const deps = results.dependenciesList;
      if (dlist?.length) {
        pkgList = pkgList.concat(dlist);
    ...
    if (hasAnyProjectType(["oci", "js", "mcp", "ai-skill"], options)) {
      ...
      if (bomData?.bomJson?.components?.length) {
        if (exactAiInventoryType) {
          ...
          thoughtLog(
            `I found ${bomData.bomJson.components.length} JavaScript packages. Let's keep looking.`,
          );
        }
        if (DEBUG_MODE) {
          console.log(
...
export async function createBom(path, options) {
  ...
  // Enable babel-based import analysis by default for evidence.occurrences
  if (options.noBabel === undefined) {
    options.noBabel = false;
  }
  // Ensure specVersion defaults to 1.5+ for evidence.occurrences support
  if (!options.specVersion || options.specVersion < 1.5) {
    options.specVersion = 1.5;
  }
  let exportData;
