      ...
      if (comp.properties) {
        if (existingComponent.properties) {
          for (const newprop of comp.properties) {
            // SrcFile chains (Array<Array<string>>) from different manifests
            // must be merged into a single SrcFile property, not stored as
            // duplicate properties. Each inner array is one full resolution
            // path; we keep distinct paths and drop exact duplicates.
            if (
              newprop.name === "SrcFile" &&
              Array.isArray(newprop.value) &&
              Array.isArray(newprop.value[0])
            ) {
              const existingSrcFile = existingComponent.properties.find(
                (prop) =>
                  prop.name === "SrcFile" &&
                  Array.isArray(prop.value) &&
                  Array.isArray(prop.value[0]),
              );
              if (existingSrcFile) {
                const seen = new Set(
                  existingSrcFile.value.map((chain) => JSON.stringify(chain)),
                );
                for (const chain of newprop.value) {
                  const key = JSON.stringify(chain);
                  if (!seen.has(key)) {
                    existingSrcFile.value.push(chain);
                    seen.add(key);
                  }
                }
                continue;
