...
it("parse gradle dependencies", async () => {
  ...
  assert.deepStrictEqual(parsedList.pkgList[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: [["build.gradle"]],  // Relative path to build file (direct dep, no parents)
      },
      {
  ...
  // Test that transitive dependencies have full chain tracking
  // Find org.bouncycastle:bcprov-jdk15on which is 4 levels deep
  const bouncyCastle = parsedList.pkgList.find(p => 
    p.group === "org.bouncycastle" && p.name === "bcprov-jdk15on"
  );
  assert.ok(bouncyCastle, "Should find bouncycastle transitive dependency");
  assert.ok(Array.isArray(bouncyCastle.properties), "Should have properties array");
  const srcFileProp = bouncyCastle.properties.find(p => p.name === "SrcFile");
  assert.ok(srcFileProp, "Should have SrcFile property");
  assert.ok(Array.isArray(srcFileProp.value), "SrcFile should be array of arrays");
  assert.ok(srcFileProp.value.length >= 1, "Should have at least one chain");
  // Check first chain
  const firstChain = srcFileProp.value[0];
  assert.ok(Array.isArray(firstChain), "Each chain should be an array");
  assert.deepStrictEqual(firstChain[0], "build.gradle", "Chain should start with build file (relative)");
  assert.ok(firstChain.length > 3, "Transitive chain should include intermediate deps");
  // Chain should include the dependency path (now in PURL format): org.web3j:core -> org.web3j:abi -> org.web3j:utils -> bcprov-jdk15on
  assert.ok(firstChain.some(v => v.includes("org.web3j") && v.includes("core")), "Chain should include parent dep");
  assert.ok(firstChain.some(v => v.includes("org.web3j") && v.includes("abi")), "Chain should include grandparent dep");
  assert.ok(firstChain.some(v => v.includes("org.web3j") && v.includes("utils")), "Chain should include great-grandparent dep");
  // Verify package itself is NOT in chain
  assert.ok(!firstChain.some(v => v.includes("bcprov-jdk15on")), "Package should NOT be in its own chain");
  ...
  parsedList = await parseGradleDep(
    ...
    modulesMap,
    "./test/data",
    "./test/data/build.gradle",
  );
  assert.deepStrictEqual(parsedList.pkgList.length, 104);
  assert.deepStrictEqual(parsedList.dependenciesList.length, 105);
  
  // Check first package - runner appears in multiple configurations, so SrcFile is array of arrays
  const runner = parsedList.pkgList[0];
  assert.deepStrictEqual(runner.group, "com.android.support.test");
  assert.deepStrictEqual(runner.name, "runner");
  assert.deepStrictEqual(runner.version, "1.0.2");
  assert.deepStrictEqual(runner.scope, "optional");
  assert.ok(runner.properties, "Should have properties");
  
  const runnerSrcFile = runner.properties.find(p => p.name === "SrcFile");
  assert.ok(runnerSrcFile, "Should have SrcFile property");
  assert.ok(Array.isArray(runnerSrcFile.value), "SrcFile should be array for multi-config dep");
  // Runner appears in multiple configs and as both direct and transitive dep
  assert.ok(runnerSrcFile.value.length >= 2, "Should have multiple resolution paths");
  // All chains should start with build file (normalized to relative path)
  assert.ok(runnerSrcFile.value.every(chain => 
    Array.isArray(chain) && chain[0] === "build.gradle"
  ), "All chains should start with build file (normalized)");
  // Should have both direct (2 elements) and transitive (3+ elements) chains
  const directChains = runnerSrcFile.value.filter(c => c.length === 2);
  const transitiveChains = runnerSrcFile.value.filter(c => c.length > 2);
  assert.ok(directChains.length > 0, "Should have direct dependency chains");
  assert.ok(transitiveChains.length > 0, "Should have transitive dependency chains");
  // Check last package - androidx.core appears in one config only
  assert.deepStrictEqual(parsedList.pkgList[103], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: [["build.gradle", "releaseUnitTestRuntimeClasspath"]], // build file + config name
      },
      {
  ...
  // Check first package from gradle-out1 - appears in multiple configs with multiple resolution paths
  const firstPkg = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg.group, "org.springframework.boot");
  assert.deepStrictEqual(firstPkg.name, "spring-boot-starter-web");
  assert.deepStrictEqual(firstPkg.version, "2.2.0.RELEASE");
  
  // Check SrcFile property - should be array of arrays with PURLs
  const srcFileProp2 = firstPkg.properties?.find(p => p.name === "SrcFile");
  assert.ok(srcFileProp2, "Should have SrcFile property");
  assert.ok(Array.isArray(srcFileProp2.value), "SrcFile should be array of arrays");
  assert.ok(srcFileProp2.value.length >= 4, "Should have multiple resolution paths");
  // Each chain should start with build.gradle
  srcFileProp2.value.forEach(chain => {
    assert.deepStrictEqual(chain[0], "build.gradle", "Chain should start with build.gradle");
  });
  // Chains with parents should have PURLs (starting with pkg:)
  const chainsWithParents = srcFileProp2.value.filter(chain => chain.length > 2);
  chainsWithParents.forEach(chain => {
    for (let i = 2; i < chain.length; i++) {
      assert.ok(chain[i].startsWith("pkg:") || !chain[i].includes(':'), 
        `Chain element ${i} should be PURL or configuration name`);
    }
  });
  ...
  const logbackCore = parsedList.pkgList[parsedList.pkgList.length - 1];
  assert.deepStrictEqual(logbackCore.name, "logback-core");
  assert.deepStrictEqual(logbackCore.version, "1.4.5");
  // Check that it has SrcFile with PURLs
  const logbackSrcFile = logbackCore.properties?.find(p => p.name === "SrcFile");
  assert.ok(logbackSrcFile, "Should have SrcFile");
  assert.ok(Array.isArray(logbackSrcFile.value[0]), "Should have chain");
  assert.deepStrictEqual(logbackSrcFile.value[0][0], "build.gradle", "Should start with build.gradle");
  ...
  // Check java-client package
  const javaClient = parsedList.pkgList.find(p => p.name === "java-client");
  assert.ok(javaClient, "Should find java-client");
  assert.deepStrictEqual(javaClient.group, "io.appium");
  assert.deepStrictEqual(javaClient.version, "8.1.1");
  const javaClientSrcFile = javaClient.properties?.find(p => p.name === "SrcFile");
  assert.ok(javaClientSrcFile, "Should have SrcFile");
  assert.deepStrictEqual(javaClientSrcFile.value, [["build.gradle"]], "Direct dep should have just build.gradle");
  
  // Check selenium-support package  
  const seleniumSupport = parsedList.pkgList.find(p => p.name === "selenium-support");
  assert.ok(seleniumSupport, "Should find selenium-support");
  assert.deepStrictEqual(seleniumSupport.group, "org.seleniumhq.selenium");
  assert.deepStrictEqual(seleniumSupport.version, "4.5.0");
  const seleniumSrcFile = seleniumSupport.properties?.find(p => p.name === "SrcFile");
  assert.ok(seleniumSrcFile, "Should have SrcFile");
  assert.ok(Array.isArray(seleniumSrcFile.value[0]), "Should have chain");
  assert.deepStrictEqual(seleniumSrcFile.value[0][0], "build.gradle", "Should start with build.gradle");
  assert.ok(seleniumSrcFile.value[0][1].startsWith("pkg:maven/io.appium/java-client"), "Parent should be PURL");
  ...
  assert.deepStrictEqual(parsedList.pkgList[0].name, "selenium-remote-driver");
  assert.deepStrictEqual(parsedList.pkgList[0].version, "4.5.0");
  ...
  assert.deepStrictEqual(parsedList.pkgList.length, 1);
  assert.deepStrictEqual(parsedList.pkgList[0].name, "selenium-api");
  assert.deepStrictEqual(parsedList.pkgList[0].version, "4.5.0");
  ...
  assert.deepStrictEqual(parsedList.pkgList.length, 1);
  assert.deepStrictEqual(parsedList.pkgList[0].name, "appcompat");
  assert.deepStrictEqual(parsedList.pkgList[0].version, "1.2.0");
...
it("parse maven tree with srcChain", () => {
  // Test with pomFile to verify srcChain tracking
  const parsedList = parseMavenTree(
    readFileSync("./test/data/mvn-dep-tree-simple.txt", {
      encoding: "utf-8",
    }),
    "./test/data/pom.xml"
  ...
  // Test direct dependency - should have chain: [pom.xml] (no parents)
  const jacksonDatabind = parsedList.pkgList.find(
    pkg => pkg.name === "jackson-databind" && pkg.version === "2.9.2"
  );
  assert.ok(jacksonDatabind, "Should find jackson-databind");
  assert.ok(jacksonDatabind.properties, "Should have properties");
  const srcFileProp = jacksonDatabind.properties.find(p => p.name === "SrcFile");
  assert.ok(srcFileProp, "Should have SrcFile property");
  assert.ok(Array.isArray(srcFileProp.value), "SrcFile should be array");
  assert.ok(Array.isArray(srcFileProp.value[0]), "SrcFile should be array of arrays");
  assert.deepStrictEqual(srcFileProp.value[0].length, 1, "Direct dep chain should have just pom file");
  assert.deepStrictEqual(srcFileProp.value[0][0], "pom.xml", "Should be relative path");
  
  // Test transitive dependency - should have longer chain with parent PURL
  const jacksonAnnotations = parsedList.pkgList.find(
    pkg => pkg.name === "jackson-annotations"
  );
  assert.ok(jacksonAnnotations, "Should find jackson-annotations (transitive)");
  const transSrcFileProp = jacksonAnnotations.properties.find(p => p.name === "SrcFile");
  assert.ok(transSrcFileProp, "Transitive dep should have SrcFile property");
  assert.ok(Array.isArray(transSrcFileProp.value[0]), "Should be array of arrays");
  assert.deepStrictEqual(transSrcFileProp.value[0][0], "pom.xml", "Should start with pom file (relative)");
  assert.deepStrictEqual(transSrcFileProp.value[0].length, 2, "Transitive dep should have pom + parent");
  assert.ok(transSrcFileProp.value[0][1].includes("pkg:maven/"), "Parent should be PURL");
  assert.ok(transSrcFileProp.value[0][1].includes("jackson-databind"), "Parent should be jackson-databind");
  assert.ok(!transSrcFileProp.value[0].join(',').includes("jackson-annotations"), "Should NOT include itself in chain");
});
...
it("parseGoSumData", async () => {
  ...
  // Check first package properties individually
  assert.deepStrictEqual(dep_list[0].group, "");
  assert.deepStrictEqual(dep_list[0].name, "google.golang.org/grpc");
  assert.deepStrictEqual(dep_list[0].version, "v1.21.0");
  assert.deepStrictEqual(dep_list[0]._integrity, "sha256-oYelfM1adQP15Ek0mdvEgi9Df8B9CZIaU1084ijfRaM=");
  assert.deepStrictEqual(dep_list[0]["bom-ref"], "pkg:golang/google.golang.org/grpc@v1.21.0");
  assert.deepStrictEqual(dep_list[0].purl, "pkg:golang/google.golang.org/grpc@v1.21.0");
  
  // Check SrcFile is array of arrays
  const srcFileProp = dep_list[0].properties.find(p => p.name === "SrcFile");
  assert.ok(srcFileProp, "Should have SrcFile property");
  assert.ok(Array.isArray(srcFileProp.value), "SrcFile should be array");
  assert.ok(Array.isArray(srcFileProp.value[0]), "SrcFile should be array of arrays");
  assert.deepStrictEqual(srcFileProp.value[0][0], "./test/gomod/go.sum");
  
  // Check second package properties individually
  assert.deepStrictEqual(dep_list[1].group, "");
  assert.deepStrictEqual(dep_list[1].name, "github.com/spf13/cobra");
  assert.deepStrictEqual(dep_list[1].version, "v1.0.0");
  assert.deepStrictEqual(dep_list[1]._integrity, "sha256-/6GTrnGXV9HjY+aR4k0oJ5tcvakLuG6EuKReYlHNrgE=");
  assert.deepStrictEqual(dep_list[1]["bom-ref"], "pkg:golang/github.com/spf13/cobra@v1.0.0");
  assert.deepStrictEqual(dep_list[1].purl, "pkg:golang/github.com/spf13/cobra@v1.0.0");
  
  // Check third package properties individually
  assert.deepStrictEqual(dep_list[2].group, "");
  assert.deepStrictEqual(dep_list[2].name, "github.com/spf13/viper");
  assert.deepStrictEqual(dep_list[2].version, "v1.0.2");
  assert.deepStrictEqual(dep_list[2]._integrity, "sha256-A8kyI5cUJhb8N+3pkfONlcEcZbueH6nhAm0Fq7SrnBM=");
  assert.deepStrictEqual(dep_list[2]["bom-ref"], "pkg:golang/github.com/spf13/viper@v1.0.2");
  assert.deepStrictEqual(dep_list[2].purl, "pkg:golang/github.com/spf13/viper@v1.0.2");
  
  // Check fourth package properties individually
  assert.deepStrictEqual(dep_list[3].group, "");
  assert.deepStrictEqual(dep_list[3].name, "github.com/stretchr/testify");
  assert.deepStrictEqual(dep_list[3].version, "v1.6.1");
  assert.deepStrictEqual(dep_list[3]._integrity, "sha256-6Fq8oRcR53rry900zMqJjRRixrwX3KX962/h/Wwjteg=");
  assert.deepStrictEqual(dep_list[3]["bom-ref"], "pkg:golang/github.com/stretchr/testify@v1.6.1");
  assert.deepStrictEqual(dep_list[3].purl, "pkg:golang/github.com/stretchr/testify@v1.6.1");
...
it("parse go version data", async () => {
  ...
  assert.deepStrictEqual(dep_list[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: "./test/data/goversion2.txt",
      },
    ],
  });
});
...
it("parse cargo lock", async () => {
  ...
  // Check first package properties individually
  assert.deepStrictEqual(dep_list[0].type, "library");
  assert.deepStrictEqual(dep_list[0].group, "");
  assert.deepStrictEqual(dep_list[0]["bom-ref"], "pkg:cargo/abscissa_core@0.5.2");
  assert.deepStrictEqual(dep_list[0].purl, "pkg:cargo/abscissa_core@0.5.2");
  assert.deepStrictEqual(dep_list[0].name, "abscissa_core");
  assert.deepStrictEqual(dep_list[0].version, "0.5.2");
  assert.deepStrictEqual(dep_list[0].hashes[0].alg, "SHA-256");
  assert.deepStrictEqual(dep_list[0].hashes[0].content, "6a07677093120a02583717b6dd1ef81d8de1e8d01bd226c83f0f9bdf3e56bb3a");
  assert.deepStrictEqual(dep_list[0].evidence.identity.field, "purl");
  assert.deepStrictEqual(dep_list[0].evidence.identity.confidence, 0.6);
  ...
  assert.ok(Array.isArray(srcFileProp.value), "SrcFile should be array");
  assert.ok(Array.isArray(srcFileProp.value[0]), "SrcFile should be array of arrays");
  assert.deepStrictEqual(srcFileProp.value[0][0], "../Cargo.lock");

  dep_list = await parseCargoData("./test/data/Cargom.lock");
  assert.deepStrictEqual(dep_list.length, 243);

  // Check second file's first package
  assert.deepStrictEqual(dep_list[0].type, "library");
  assert.deepStrictEqual(dep_list[0].group, "");
  assert.deepStrictEqual(dep_list[0]["bom-ref"], "pkg:cargo/actix-codec@0.3.0");
  assert.deepStrictEqual(dep_list[0].purl, "pkg:cargo/actix-codec@0.3.0");
  assert.deepStrictEqual(dep_list[0].name, "actix-codec");
  assert.deepStrictEqual(dep_list[0].version, "0.3.0");
  assert.deepStrictEqual(dep_list[0].hashes[0].alg, "SHA-256");
  assert.deepStrictEqual(dep_list[0].hashes[0].content, "78d1833b3838dbe990df0f1f87baf640cf6146e898166afe401839d1b001e570");
...
it("parse conan data", () => {
  ...
  conanLockData = parseConanLockData(
    ...
    "./test/data/conan-v1-for-reference.lock",
  );
  ...
  assert.deepStrictEqual(conanLockData.pkgList[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: [["conan-v1-for-reference.lock"]],
      },
    ],
  });
  ...
  conanLockData = parseConanLockData(
    ...
    "./test/data/conan-v1-with-nested-deps.lock",
  );
  ...
  assert.deepStrictEqual(conanLockData.pkgList[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: [["conan-v1-with-nested-deps.lock"]],
      },
    ],
  });
    ...
    properties: [
      {
        name: "SrcFile",
        value: [
          ["conan-v1-with-nested-deps.lock", "pkg:conan/grpc@1.50.1"],
        ],
      },
    ],
  });
    ...
    properties: [
      {
        ...
        value: [
          ["conan-v1-with-nested-deps.lock", "pkg:conan/grpc@1.50.1"],
          [
            "conan-v1-with-nested-deps.lock",
  ...
  assert.deepStrictEqual(dep_list[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: "./test/data/cmakes/conanfile.txt",
      },
    ],
  });
  dep_list = parseConanData(
    readFileSync("./test/data/cmakes/conanfile1.txt", { encoding: "utf-8" }),
    "./test/data/cmakes/conanfile1.txt",
  );
  ...
  assert.deepStrictEqual(dep_list[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: "./test/data/cmakes/conanfile1.txt",
      },
    ],
  });
});
...
it("parse conan data where packages use custom user/channel", () => {
  const conanLockData = parseConanLockData(
    ...
    "./test/data/conan.with_custom_pkg_user_channel.lock",
  );
  ...
  assert.deepStrictEqual(conanLockData.pkgList[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: [["conan.with_custom_pkg_user_channel.lock"]],
      },
    ],
  });
  assert.deepStrictEqual(conanLockData.pkgList[1], {
  ...
  const dep_list = parseConanData(
    ...
    "./test/data/conanfile.with_custom_pkg_user_channel.txt",
  );
  ...
  assert.deepStrictEqual(dep_list[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: "./test/data/conanfile.with_custom_pkg_user_channel.txt",
      },
    ],
  });
  assert.deepStrictEqual(dep_list[1], {
...
it("parse project.assets.json", () => {
  ...
  assert.deepStrictEqual(dep_list["pkgList"][0], {
    ...
    version: "0.0.0",
    properties: [
      { name: "SrcFile", value: [["project.assets.json"]] },
    ],
  });
  ...
  assert.deepStrictEqual(dep_list["pkgList"][0], {
    ...
    version: "1.0.0",
    properties: [
      { name: "SrcFile", value: [["project.assets1.json"]] },
    ],
  });
  /*
...
it("parse packages.lock.json", () => {
  ...
  assert.deepStrictEqual(dep_list["pkgList"][0], {
    ...
    properties: [{ name: "SrcFile", value: [["packages.lock.json"]] }],
    evidence: {
      identity: {
        field: "purl",
        confidence: 1,
  ...
  assert.deepStrictEqual(dep_list["pkgList"][0], {
    ...
    properties: [{ name: "SrcFile", value: [["packages2.lock.json"]] }],
    evidence: {
      identity: {
        field: "purl",
        confidence: 1,
    ...
    properties: [
      {
        name: "SrcFile",
        value: [
          ["packages3.lock.json", "pkg:nuget/Argu@4.0.0"],
          ["packages3.lock.json", "pkg:nuget/Froto.Parser"],
          [
            "packages3.lock.json",
            "pkg:nuget/Froto.Parser",
            "pkg:nuget/FParsec@1.0.3",
          ],
        ],
      },
    ],
...
it("parse paket.lock", () => {
  ...
  assert.deepStrictEqual(dep_list.pkgList[0], {
    ...
    properties: [{ name: "SrcFile", value: [["paket.lock"]] }],
    evidence: {
      identity: {
        field: "purl",
        confidence: 1,
...
it("parseCsProjAssetsData populates SrcFile multi-source dependency chains", () => {
  const file = "./test/data/csproj-assets-multi-source.json";
  const r = parseCsProjAssetsData(readFileSync(file, "utf-8"), file);
  const byName = Object.fromEntries(r.pkgList.map((p) => [p.name, p]));
  const chain = (name) =>
    byName[name].properties.find((p) => p.name === "SrcFile").value;
  for (const pkg of r.pkgList) {
    const sf = pkg.properties.find((p) => p.name === "SrcFile");
    assert.ok(Array.isArray(sf.value));
    assert.ok(Array.isArray(sf.value[0]));
    for (const c of sf.value) {
      assert.deepStrictEqual(c[0], "csproj-assets-multi-source.json");
      assert.ok(!c.includes(pkg["bom-ref"]));
    }
  }
  assert.deepStrictEqual(chain("SampleApp"), [
    ["csproj-assets-multi-source.json"],
  ]);
  // SharedCore reached via 3 routes (DevTool, FeatureA, FeatureB) under SampleApp
  const sharedCore = chain("SharedCore");
  assert.deepStrictEqual(sharedCore.length, 3);
  const sharedKeys = new Set(sharedCore.map((c) => c.join("|")));
  for (const mid of ["DevTool@0.9.0", "FeatureA@1.2.0", "FeatureB@1.3.0"]) {
    assert.ok(
      sharedKeys.has(
        `csproj-assets-multi-source.json|pkg:nuget/SampleApp@1.0.0|pkg:nuget/${mid}`,
      ),
      `expected route through ${mid}`,
    );
  }
  const lowLevel = chain("LowLevelUtil");
  assert.deepStrictEqual(lowLevel.length, 3);
  for (const c of lowLevel) {
    assert.deepStrictEqual(c[c.length - 1], "pkg:nuget/SharedCore@2.5.1");
  }
});

it("parseCsPkgLockData populates SrcFile multi-source dependency chains", () => {
  const file = "./test/data/packages-multi-source.lock.json";
  const r = parseCsPkgLockData(readFileSync(file, "utf-8"), file);
  const byName = Object.fromEntries(r.pkgList.map((p) => [p.name, p]));
  const chain = (name) =>
    byName[name].properties.find((p) => p.name === "SrcFile").value;
  for (const pkg of r.pkgList) {
    ...
    assert.ok(Array.isArray(sf.value[0]));
    for (const c of sf.value) {
      assert.deepStrictEqual(c[0], "packages-multi-source.lock.json");
      assert.ok(!c.includes(pkg["bom-ref"]));
    }
  }
  for (const root of ["FeatureA", "FeatureB", "DevTool"]) {
    assert.deepStrictEqual(chain(root), [
      ["packages-multi-source.lock.json"],
    ]);
  }
  const sharedCore = chain("SharedCore");
  assert.deepStrictEqual(sharedCore.length, 3);
...
it("parsePaketLockData populates SrcFile multi-source dependency chains", () => {
  const file = "./test/data/paket-multi-source.lock";
  const r = parsePaketLockData(readFileSync(file, "utf-8"), file);
  const byName = Object.fromEntries(r.pkgList.map((p) => [p.name, p]));
  const chain = (name) =>
    byName[name].properties.find((p) => p.name === "SrcFile").value;
  for (const pkg of r.pkgList) {
    ...
    assert.ok(Array.isArray(sf.value[0]));
    for (const c of sf.value) {
      assert.deepStrictEqual(c[0], "paket-multi-source.lock");
      assert.ok(!c.includes(pkg["bom-ref"]));
    }
  }
  for (const root of ["FeatureA", "FeatureB", "DevTool"]) {
    assert.deepStrictEqual(chain(root), [["paket-multi-source.lock"]]);
  }
  assert.deepStrictEqual(chain("SharedCore").length, 3);
  const lowLevel = chain("LowLevelUtil");
  assert.deepStrictEqual(lowLevel.length, 3);
  for (const c of lowLevel) {
    assert.deepStrictEqual(c[c.length - 1], "pkg:nuget/SharedCore@2.5.1");
...
it("parseConanLockData populates SrcFile multi-source dependency chains", () => {
  const file = "./test/data/conan-multi-source.lock";
  const r = parseConanLockData(readFileSync(file, "utf-8"), file);
  const byName = Object.fromEntries(r.pkgList.map((p) => [p.name, p]));
  const chain = (name) =>
    byName[name].properties.find((p) => p.name === "SrcFile").value;
  for (const pkg of r.pkgList) {
    ...
    assert.ok(Array.isArray(sf.value[0]));
    for (const c of sf.value) {
      assert.deepStrictEqual(c[0], "conan-multi-source.lock");
      assert.ok(!c.includes(pkg["bom-ref"]));
    }
  }
  for (const root of ["feature-a", "feature-b", "dev-tool"]) {
    assert.deepStrictEqual(chain(root), [["conan-multi-source.lock"]]);
  }
  // shared-core: reached through feature-a, feature-b, and dev-tool — each
  // hangs off the parent component (synthetic) so chains do NOT include any
  // __conan_parent__ segment.
  const sharedCore = chain("shared-core");
  assert.deepStrictEqual(sharedCore.length, 3);
  for (const c of sharedCore) {
    assert.ok(!c.some((seg) => seg.startsWith("__conan_parent__")));
  }
  const lowLevel = chain("low-level-util");
  assert.deepStrictEqual(lowLevel.length, 3);
  for (const c of lowLevel) {
    assert.deepStrictEqual(c[c.length - 1], "pkg:conan/shared-core@2.5.1");
  }
});
...
it("parsePkgLock with npm: alias", async () => {
  const parsedList = await parsePkgLock(
    "./test/data/package-json/alias-test/package-lock.json",
  );
  const deps = parsedList.pkgList;
  
  // Should have 2 versions of react
  const reactVersions = deps.filter(d => d.name === "react");
  assert.deepStrictEqual(reactVersions.length, 2, "Should have 2 react entries");
  
  // Find the aliased version (17.0.2)
  const reactOld = reactVersions.find(d => d.version === "17.0.2");
  assert.ok(reactOld, "Should have react@17.0.2");
  
  // Check that alias property exists
  const aliasProperty = reactOld.properties?.find(p => p.name === "cdx:npm:package:alias");
  assert.ok(aliasProperty, "react@17.0.2 should have alias property");
  assert.deepStrictEqual(aliasProperty.value, "react-old", "Alias should be 'react-old'");
  
  // Find the regular version (18.2.0)
  const reactNew = reactVersions.find(d => d.version === "18.2.0");
  assert.ok(reactNew, "Should have react@18.2.0");
  
  // Check that regular version does NOT have alias property
  const noAliasProperty = reactNew.properties?.find(p => p.name === "cdx:npm:package:alias");
  assert.ok(!noAliasProperty, "react@18.2.0 should not have alias property");
});
...
it("parsePkgLock SrcFile chain tracking", async () => {
  const projectRoot = "./test/data/package-json";
  const parsedList = await parsePkgLock(
    "./test/data/package-json/v1/package-lock.json",
    {},
    projectRoot,
  ...
  // All non-root packages should have SrcFile property
  const packagesWithSrcFile = deps.filter(d => {
    const srcFile = d.properties?.find(p => p.name === "SrcFile");
    return srcFile !== undefined;
  });
  
  assert.ok(packagesWithSrcFile.length > 0, "Should have packages with SrcFile property");
  
  // Find a direct dependency
  const directDep = deps.find(d => d.name === "react");
  assert.ok(directDep, "Should find react (direct dependency)");
  
  const directSrcFile = directDep.properties?.find(p => p.name === "SrcFile");
  assert.ok(directSrcFile, "Direct dependency should have SrcFile property");
  assert.ok(Array.isArray(directSrcFile.value), "SrcFile should be an array");
  assert.ok(Array.isArray(directSrcFile.value[0]), "SrcFile should be array of arrays");
  
  const directChain = directSrcFile.value[0];
  assert.deepStrictEqual(directChain.length, 1, "Direct dependency chain should only have lock file");
  assert.deepStrictEqual(directChain[0], "v1/package-lock.json", "Chain should start with relative path to lock file");
  
  // Find a transitive dependency (2 levels deep)
  const transitiveDep = deps.find(d => d.name === "fbjs");
  assert.ok(transitiveDep, "Should find fbjs (transitive dependency)");
  
  const transitiveSrcFile = transitiveDep.properties?.find(p => p.name === "SrcFile");
  assert.ok(transitiveSrcFile, "Transitive dependency should have SrcFile property");
  assert.ok(Array.isArray(transitiveSrcFile.value[0]), "SrcFile should be array of arrays");
  
  const transitiveChain = transitiveSrcFile.value[0];
  assert.deepStrictEqual(transitiveChain.length, 2, "Transitive dependency chain should have lock file + 1 parent");
  assert.deepStrictEqual(transitiveChain[0], "v1/package-lock.json", "Chain should start with relative lock file path");
  assert.ok(transitiveChain[1].startsWith("pkg:npm/"), "Second element should be parent PURL");
  assert.ok(transitiveChain[1].includes("react"), "Parent PURL should be react");
  
  // Package itself should NOT be in the chain
  assert.ok(!transitiveChain.some(item => item.includes("fbjs")), "Package itself should NOT be in chain");
  
  // Find a deeper transitive dependency (3 levels deep)
  const deepTransitive = deps.find(d => d.name === "core-js" && d.version === "1.2.7");
  assert.ok(deepTransitive, "Should find core-js (3 levels deep)");
  
  const deepSrcFile = deepTransitive.properties?.find(p => p.name === "SrcFile");
  const deepChain = deepSrcFile.value[0];
  assert.deepStrictEqual(deepChain.length, 3, "Deep transitive should have lock file + 2 parents");
  assert.deepStrictEqual(deepChain[0], "v1/package-lock.json", "Chain should start with relative lock file path");
  assert.ok(deepChain[1].startsWith("pkg:npm/"), "Second element should be parent PURL");
  assert.ok(deepChain[2].startsWith("pkg:npm/"), "Third element should be grandparent PURL");
  
  // Package itself should NOT be in the chain
  assert.ok(!deepChain.some(item => item.includes("core-js")), "Package itself should NOT be in chain");
});

it("parsePkgLock SrcFile chain without project root", async () => {
  // Test backward compatibility - when no project root is provided
  const parsedList = await parsePkgLock(
    "./test/data/package-json/v1/package-lock.json",
    {},
  );
  ...
  const directDep = deps.find(d => d.name === "react");
  const srcFile = directDep.properties?.find(p => p.name === "SrcFile");
  const chain = srcFile.value[0];
  
  // Without project root, should be relative to lock file directory
  assert.deepStrictEqual(chain[0], "package-lock.json", "Should default to lock file name when no project root");
});
...
it("runPigarInVenv integration test", async () => {
  const { runPigarInVenv } = await import("../helpers/utils.js");
  const { mkdtempSync, existsSync, readFileSync: readFile, rmSync } = await import("node:fs");
  const { join } = await import("node:path");
  const { getTmpDir } = await import("../helpers/utils.js");
  
  // Create a temporary venv directory
  const tempVenvDir = mkdtempSync(join(getTmpDir(), "test-pigar-venv-"));
  const setupProjectDir = "./test/data/pysetup-project";
  const generatedReqFile = join(setupProjectDir, "extracted-requirements.txt");
  
  try {
    // Clean up any existing extracted-requirements.txt from previous test runs
    if (existsSync(generatedReqFile)) {
      rmSync(generatedReqFile, { force: true });
    }
    
    // Run pigar in the isolated venv
    const result = await runPigarInVenv(setupProjectDir, tempVenvDir);
    
    // Fail the test if pigar didn't work
    assert.ok(result?.success, `Pigar failed: ${result?.error} - ${result?.details}`);
    
    // Check if extracted-requirements.txt was generated
    assert.ok(existsSync(generatedReqFile), "extracted-requirements.txt should be generated");
    
    // Read and verify the generated extracted-requirements.txt
    const reqContent = readFile(generatedReqFile, "utf-8");
    
    // Verify that it contains the requests package (imported in main.py)
    assert.ok(
      reqContent.includes("requests"),
      "Generated extracted-requirements.txt should include requests package"
    );
    
    // Verify that pigar itself is NOT in the requirements (it's in isolated venv)
    assert.ok(
      !reqContent.includes("pigar"),
      "pigar should not be in the project's extracted-requirements.txt"
    );
    
    // Parse the generated requirements file
    const { parseReqFile } = await import("../helpers/utils.js");
    const parsedDeps = await parseReqFile(generatedReqFile, false);
    
    // Verify we got dependencies
    assert.ok(parsedDeps.length > 0, "Should parse dependencies from generated file");
      
      // Verify requests is in the parsed list
      const requestsDep = parsedDeps.find(d => d.name === "requests");
      assert.ok(requestsDep, "Should find requests in parsed dependencies");
      assert.ok(requestsDep.version, "requests should have a version");
    ...
    if (existsSync(tempVenvDir)) {
      rmSync(tempVenvDir, { recursive: true, force: true });
    }
  }
});
...
it("parsePnpmLock", async () => {
  ...
  // Check first package properties
  let firstPkg = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg.name, "code-frame");
  assert.deepStrictEqual(firstPkg.group, "@babel");
  assert.deepStrictEqual(firstPkg.version, "7.10.1");
  
  // SrcFile is now array of arrays
  let srcFile = firstPkg.properties?.find(p => p.name === "SrcFile");
  assert.ok(srcFile, "Should have SrcFile property");
  assert.ok(Array.isArray(srcFile.value), "SrcFile should be array");
  assert.ok(Array.isArray(srcFile.value[0]), "SrcFile should be array of arrays");
  ...
  firstPkg = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg.name, "code-frame");
  assert.deepStrictEqual(firstPkg.group, "@babel");
  assert.deepStrictEqual(firstPkg.version, "7.16.7");
  parsedList = await parsePnpmLock("./test/data/pnpm-lock2.yaml");
  assert.deepStrictEqual(parsedList.pkgList.length, 7);
  assert.deepStrictEqual(parsedList.dependenciesList.length, 7);
  
  firstPkg = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg.name, "ansi-regex");
  assert.deepStrictEqual(firstPkg.version, "2.1.1");
  ...
  firstPkg = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg.name, "fs.scandir");
  assert.deepStrictEqual(firstPkg.group, "@nodelib");
  assert.deepStrictEqual(firstPkg.version, "2.1.5");
  ...
  assert.deepStrictEqual(firstPkg.name, "code-frame");
  assert.deepStrictEqual(firstPkg.group, "@babel");
  assert.deepStrictEqual(firstPkg.version, "7.18.6");

  let lastPkg = parsedList.pkgList[parsedList.pkgList.length - 1];
  assert.deepStrictEqual(lastPkg.name, "yargs");
  assert.deepStrictEqual(lastPkg.version, "17.7.1");
...
it("parsePnpmLock SrcFile chain tracking", async () => {
  const projectRoot = "./test/data";
  const pnpmLockFile = "./test/data/pnpm-lock.yaml";
  
  const result = await parsePnpmLock(pnpmLockFile, null, [], {}, {}, {}, {}, projectRoot);
  const { pkgList, dependenciesList } = result;
  
  // All packages should have SrcFile property
  assert.ok(pkgList.length > 0, "Should have parsed packages");
  const withSrcFile = pkgList.filter(p => p.properties?.find(pr => pr.name === "SrcFile"));
  assert.deepStrictEqual(withSrcFile.length, pkgList.length, "All packages should have SrcFile");
  
  // Find a direct dependency (root with no parents)
  const directDep = pkgList.find(p => {
    const srcFile = p.properties?.find(pr => pr.name === "SrcFile");
    return srcFile && srcFile.value.length === 1 && srcFile.value[0].length === 1;
  });
  assert.ok(directDep, "Should find at least one direct dependency");
  
  const directDepSrcFile = directDep.properties.find(p => p.name === "SrcFile");
  assert.ok(Array.isArray(directDepSrcFile.value), "SrcFile should be array");
  assert.ok(Array.isArray(directDepSrcFile.value[0]), "SrcFile should be array of arrays");
  assert.deepStrictEqual(directDepSrcFile.value[0][0], "pnpm-lock.yaml", "Direct dep chain should be just lock file");
  
  // Find a transitive dependency (has parent)
  const transitiveDep = pkgList.find(p => {
    const srcFile = p.properties?.find(pr => pr.name === "SrcFile");
    return srcFile && srcFile.value.length > 0 && srcFile.value[0].length > 1;
  });
  assert.ok(transitiveDep, "Should find at least one transitive dependency");
  
  const transitiveSrcFile = transitiveDep.properties.find(p => p.name === "SrcFile");
  assert.ok(transitiveSrcFile.value[0].length >= 2, "Transitive should have lock file + parent(s)");
  assert.deepStrictEqual(transitiveSrcFile.value[0][0], "pnpm-lock.yaml", "Chain should start with lock file");
  assert.ok(transitiveSrcFile.value[0][1].startsWith("pkg:"), "Second element should be parent PURL");
  
  // Verify package itself is NOT in its chain
  for (const pkg of pkgList) {
    const srcFile = pkg.properties?.find(p => p.name === "SrcFile");
    if (srcFile && srcFile.value.length > 0) {
      for (const chain of srcFile.value) {
        for (let i = 1; i < chain.length; i++) { // Skip first element (lock file)
          assert.notDeepStrictEqual(chain[i], pkg["bom-ref"], `Package ${pkg["bom-ref"]} should not be in its own chain`);
        }
      }
    }
  }
  
  // Test packages with multiple paths (same package reached via different parents)
  const multiPathPkg = pkgList.find(p => {
    const srcFile = p.properties?.find(pr => pr.name === "SrcFile");
    return srcFile && srcFile.value.length > 1;
  });
  if (multiPathPkg) {
    const multiPathSrcFile = multiPathPkg.properties.find(p => p.name === "SrcFile");
    assert.ok(multiPathSrcFile.value.length > 1, "Package should have multiple chains");
    
    // All chains should start with lock file
    for (const chain of multiPathSrcFile.value) {
      assert.deepStrictEqual(chain[0], "pnpm-lock.yaml", "All chains should start with lock file");
    }
  }
});
...
it("pnpmMetadata integration with parsePnpmLock", async () => {
  ...
  // Check that if any package has an alias, it uses the standardized property name
  const packagesWithAlias = parsedList.pkgList.filter((pkg) =>
    pkg.properties?.some((p) => p.name === "cdx:npm:package:alias"),
  );
  // If aliases exist, verify the property name is correct (not the old cdx:pnpm:alias)
  if (packagesWithAlias.length > 0) {
    for (const pkg of packagesWithAlias) {
      const aliasProp = pkg.properties.find((p) => p.name === "cdx:npm:package:alias");
      assert.ok(aliasProp, "Alias property should use cdx:npm:package:alias");
      assert.ok(aliasProp.value, "Alias property should have a value");
      // Ensure old property name is not used
      const oldProp = pkg.properties.find((p) => p.name === "cdx:pnpm:alias");
      assert.ok(!oldProp, "Old cdx:pnpm:alias property should not be used");
    }
  }
});

it("parsePnpmLock comprehensive alias test", async () => {
  // Test all edge cases from Berry lock file patterns
  const parsedList = await parsePnpmLock("./test/data/pnpm_locks/pnpm-alias-comprehensive.yaml");
  
  // Test 1: Scoped to non-scoped alias (@babel/code-frame -> babel-code-frame)
  const babelCodeFrame = parsedList.pkgList.find(
    (p) => p.name === "babel-code-frame" && p.version === "6.26.0"
  );
  assert.ok(babelCodeFrame, "babel-code-frame@6.26.0 should exist");
  const babelCodeFrameAlias = babelCodeFrame.properties?.find(
    (p) => p.name === "cdx:npm:package:alias"
  );
  assert.ok(babelCodeFrameAlias, "babel-code-frame should have alias property");
  assert.strictEqual(
    babelCodeFrameAlias.value,
    "@babel/code-frame",
    "Alias should be @babel/code-frame"
  );
  
  // Test 2: Scoped to scoped alias (@babel/register -> @babel/helper-validator-identifier)
  const helperValidatorId = parsedList.pkgList.find(
    (p) => p.name === "helper-validator-identifier" && p.version === "7.28.5"
  );
  assert.ok(helperValidatorId, "@babel/helper-validator-identifier@7.28.5 should exist");
  const helperValidatorIdAlias = helperValidatorId.properties?.find(
    (p) => p.name === "cdx:npm:package:alias"
  );
  assert.ok(helperValidatorIdAlias, "helper-validator-identifier should have alias property");
  assert.strictEqual(
    helperValidatorIdAlias.value,
    "@babel/register",
    "Alias should be @babel/register"
  );
  
  // Test 3: Wildcard resolution (@types/is -> @types/abs)
  const typesAbs = parsedList.pkgList.find(
    (p) => p.name === "abs" && p.version === "1.3.4"
  );
  assert.ok(typesAbs, "@types/abs@1.3.4 should exist");
  const typesAbsAlias = typesAbs.properties?.find(
    (p) => p.name === "cdx:npm:package:alias"
  );
  assert.ok(typesAbsAlias, "abs should have alias property");
  assert.strictEqual(
    typesAbsAlias.value,
    "@types/is",
    "Alias should be @types/is"
  );
  
  // Test 4: Non-scoped to non-scoped alias (ansi-styles -> ansi-regex)
  const ansiRegex = parsedList.pkgList.find(
    (p) => p.name === "ansi-regex" && p.version === "6.2.2"
  );
  assert.ok(ansiRegex, "ansi-regex@6.2.2 should exist");
  const ansiRegexAlias = ansiRegex.properties?.find(
    (p) => p.name === "cdx:npm:package:alias"
  );
  assert.ok(ansiRegexAlias, "ansi-regex should have alias property");
  assert.strictEqual(
    ansiRegexAlias.value,
    "ansi-styles",
    "Alias should be ansi-styles"
  );
  
  // Test 5: Non-scoped to scoped alias (babel-helper-transform-fixture-test-runner -> @babel/helper-transform-fixture-test-runner)
  const helperTransformFixture = parsedList.pkgList.find(
    (p) => p.name === "helper-transform-fixture-test-runner" && p.version === "7.28.6"
  );
  assert.ok(helperTransformFixture, "@babel/helper-transform-fixture-test-runner@7.28.6 should exist");
  const helperTransformFixtureAlias = helperTransformFixture.properties?.find(
    (p) => p.name === "cdx:npm:package:alias"
  );
  assert.ok(helperTransformFixtureAlias, "helper-transform-fixture-test-runner should have alias property");
  assert.strictEqual(
    helperTransformFixtureAlias.value,
    "babel-helper-transform-fixture-test-runner",
    "Alias should be babel-helper-transform-fixture-test-runner"
  );
  
  // Test 6: Simple alias (react-old -> react@17.0.2)
  const react17 = parsedList.pkgList.find(
    (p) => p.name === "react" && p.version === "17.0.2"
  );
  assert.ok(react17, "react@17.0.2 should exist");
  const react17Alias = react17.properties?.find(
    (p) => p.name === "cdx:npm:package:alias"
  );
  assert.ok(react17Alias, "react@17.0.2 should have alias property");
  assert.strictEqual(
    react17Alias.value,
    "react-old",
    "Alias should be react-old"
  );
  
  // Test 7: Non-aliased package (react@18.3.1 should NOT have alias)
  const react18 = parsedList.pkgList.find(
    (p) => p.name === "react" && p.version === "18.3.1"
  );
  assert.ok(react18, "react@18.3.1 should exist");
  const react18Alias = react18.properties?.find(
    (p) => p.name === "cdx:npm:package:alias"
  );
  assert.ok(!react18Alias, "react@18.3.1 should NOT have alias property");
  
  // Test 8: Verify no old property names are used
  const allAliasedPackages = parsedList.pkgList.filter((pkg) =>
    pkg.properties?.some((p) => p.name === "cdx:npm:package:alias")
  );
  assert.ok(allAliasedPackages.length >= 6, "Should have at least 6 aliased packages");
  
  for (const pkg of allAliasedPackages) {
    const oldProp = pkg.properties.find((p) => p.name === "cdx:pnpm:alias");
    assert.ok(!oldProp, `${pkg.name}@${pkg.version} should not use old cdx:pnpm:alias property`);
  }
  
  // Test 9: Verify both versions of ansi-regex exist
  const ansiRegexVersions = parsedList.pkgList.filter((p) => p.name === "ansi-regex");
  assert.ok(ansiRegexVersions.length >= 2, "Should have multiple ansi-regex versions");
  const ansiRegex2 = ansiRegexVersions.find((p) => p.version === "2.1.1");
  assert.ok(ansiRegex2, "ansi-regex@2.1.1 should exist (non-aliased transitive)");
  const ansiRegex2Alias = ansiRegex2.properties?.find(
    (p) => p.name === "cdx:npm:package:alias"
  );
  assert.ok(!ansiRegex2Alias, "ansi-regex@2.1.1 should NOT have alias property");
});
...
it("parseYarnLock", async () => {
  ...
  const firstPkg = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg.name, "asap");
  assert.deepStrictEqual(firstPkg.version, "2.0.5");
  assert.deepStrictEqual(firstPkg.purl, "pkg:npm/asap@2.0.5");
  
  // Check SrcFile is now array of arrays format
  const srcFileProp = firstPkg.properties?.find(p => p.name === "SrcFile");
  assert.ok(srcFileProp, "Should have SrcFile property");
  assert.ok(Array.isArray(srcFileProp.value), "SrcFile should be an array");
  assert.ok(Array.isArray(srcFileProp.value[0]), "SrcFile should be array of arrays");
  assert.deepStrictEqual(srcFileProp.value[0][0], "yarn.lock", "First element should be lock file");
  ...
  const firstPkg2 = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg2.name, "cli");
  assert.deepStrictEqual(firstPkg2.group, "@babel");
  assert.deepStrictEqual(firstPkg2.version, "7.10.1");
  ...
  const firstPkg3 = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg3.name, "client");
  assert.deepStrictEqual(firstPkg3.group, "@apollo");
  assert.deepStrictEqual(firstPkg3.version, "3.2.5");
  ...
  // Check package properties
  const firstPkg4 = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg4.name, "artifact");
  assert.deepStrictEqual(firstPkg4.group, "@actions");
  assert.deepStrictEqual(firstPkg4.version, "0.6.1");
  ...
  // Check package properties
  const secondPkg5 = parsedList.pkgList[1];
  assert.deepStrictEqual(secondPkg5.name, "lru-cache");
  assert.deepStrictEqual(secondPkg5.version, "6.0.0");

  parsedList = await parseYarnLock("./test/data/yarn_locks/yarnv2.lock");
  assert.deepStrictEqual(parsedList.pkgList.length, 1090);
  assert.deepStrictEqual(parsedList.dependenciesList.length, 1090);
  assert.deepStrictEqual(
    isPartialTree(parsedList.dependenciesList, parsedList.pkgList.length),
    false,
  );
  ...
  // Check package properties
  const firstPkg6 = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg6.name, "slice-ansi");
  assert.deepStrictEqual(firstPkg6.group, "@arcanis");
  assert.deepStrictEqual(firstPkg6.version, "1.0.2");
  // Verify Berry lockfile dependency resolution (npm: prefix stripping)
  const yarnv2WithDeps = parsedList.dependenciesList.filter(d => d.dependsOn?.length > 0);
  assert.ok(yarnv2WithDeps.length > 400, `Berry v2 should have many populated dependsOn (got ${yarnv2WithDeps.length})`);
  assert.deepStrictEqual(parsedList.dependenciesList[0], {
    ref: "pkg:npm/@arcanis/slice-ansi@1.0.2",
    dependsOn: ["pkg:npm/grapheme-splitter@1.0.4"],
  });

  parsedList = await parseYarnLock("./test/data/yarn_locks/yarnv3.lock");
  assert.deepStrictEqual(parsedList.pkgList.length, 366);
  assert.deepStrictEqual(parsedList.dependenciesList.length, 366);
  assert.deepStrictEqual(
    isPartialTree(parsedList.dependenciesList, parsedList.pkgList.length),
    false,
  );
  ...
  // Check package properties
  const firstPkg7 = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg7.name, "code-frame");
  assert.deepStrictEqual(firstPkg7.group, "@babel");
  assert.deepStrictEqual(firstPkg7.version, "7.16.7");
  ...
  // Check package properties
  const firstPkg8 = parsedList.pkgList[0];
  assert.deepStrictEqual(firstPkg8.name, "yjs");
  assert.deepStrictEqual(firstPkg8.group, "@ac-synth");
  assert.deepStrictEqual(firstPkg8.version, "13.5.39-alpha1");
  ...
  // Updated count: Yarn 1 aliases (string-width-cjs, strip-ansi-cjs, wrap-ansi-cjs)
  // now correctly resolve to their actual package names, reducing duplicate entries
  ...
  // Updated count: includes 4 patched packages (fsevents, resolve x2, typescript)
  assert.deepStrictEqual(parsedList.pkgList.length, 1351);
  assert.deepStrictEqual(parsedList.dependenciesList.length, 1351);
  assert.deepStrictEqual(
    isPartialTree(parsedList.dependenciesList, parsedList.pkgList.length),
    false,
  );
  ...
  assert.deepStrictEqual(parsedList.pkgList.length, 1858);
  assert.deepStrictEqual(parsedList.dependenciesList.length, 1858);
  assert.deepStrictEqual(
    parsedList.pkgList[0].purl,
    "pkg:npm/%40aashutoshrathi/word-wrap@1.2.6",
  );
  ...
  // Verify Berry v4 dependency resolution (npm: prefix stripping)
  const yarnv4WithDeps = parsedList.dependenciesList.filter(d => d.dependsOn?.length > 0);
  assert.ok(yarnv4WithDeps.length > 800, `Berry v4 should have many populated dependsOn (got ${yarnv4WithDeps.length})`);
  assert.deepStrictEqual(
    parsedList.dependenciesList.find(d => d.ref === "pkg:npm/@algolia/autocomplete-core@1.7.1"),
    {
      ref: "pkg:npm/@algolia/autocomplete-core@1.7.1",
      dependsOn: ["pkg:npm/@algolia/autocomplete-shared@1.7.1"],
    },
  );
  assert.deepStrictEqual(isPartialTree(parsedList.dependenciesList), false);
  parsedList = await parseYarnLock("./test/data/yarn_locks/yarnv4.1.lock");
  // Updated count: includes 3 patched packages (fsevents, resolve, typescript)
  assert.deepStrictEqual(parsedList.pkgList.length, 861);
  assert.deepStrictEqual(parsedList.dependenciesList.length, 861);
  assert.deepStrictEqual(
    parsedList.pkgList[0].purl,
    "pkg:npm/%40aashutoshrathi/word-wrap@1.2.6",
  );
...
`;
    ...
    // The yarnLockToIdentMap function has been removed as part of the 2022 parser refactor
...
it("parseYarnLock SrcFile chain tracking", async () => {
  const projectRoot = "./test/data";
  const yarnLockFile = "./test/data/yarn_locks/yarn.lock";
  
  const result = await parseYarnLock(yarnLockFile, null, [], {}, {}, {}, projectRoot);
  const { pkgList, dependenciesList } = result;
  ...
  assert.ok(Array.isArray(directDepSrcFile.value), "SrcFile should be array");
  assert.ok(Array.isArray(directDepSrcFile.value[0]), "SrcFile should be array of arrays");
  assert.deepStrictEqual(directDepSrcFile.value[0][0], "yarn_locks/yarn.lock", "Direct dep chain should be just lock file");
  ...
  const transitiveSrcFile = transitiveDep.properties.find(p => p.name === "SrcFile");
  assert.ok(transitiveSrcFile.value[0].length >= 2, "Transitive should have lock file + parent(s)");
  assert.deepStrictEqual(transitiveSrcFile.value[0][0], "yarn_locks/yarn.lock", "Chain should start with lock file");
  assert.ok(transitiveSrcFile.value[0][1].startsWith("pkg:"), "Second element should be parent PURL");
  ...
  for (const pkg of pkgList) {
    ...
    if (srcFile && srcFile.value.length > 0) {
      for (const chain of srcFile.value) {
        for (let i = 1; i < chain.length; i++) { // Skip first element (lock file)
          assert.notDeepStrictEqual(chain[i], pkg.purl, `Package ${pkg.purl} should not be in its own chain`);
        }
      }
    }
  }
  ...
  if (multiPathPkg) {
    ...
    // All chains should start with lock file
    for (const chain of multiPathSrcFile.value) {
      assert.deepStrictEqual(chain[0], "yarn_locks/yarn.lock", "All chains should start with lock file");
    }
  }
});
...
it("parseComposerLock", () => {
    ...
    properties: [
      {
        name: "SrcFile",
        value: [
          ["composer-2.lock", "pkg:composer/amphp/byte-stream@v1.7.3"],
        ],
      },
      {
        name: "Namespaces",
    ...
    properties: [
      {
        ...
        value: [
          [
            "composer-3.lock",
            "pkg:composer/humbug/box@4.2.0",
            "pkg:composer/amphp/parallel-functions@v1.1.0",
          ],
          [
            "composer-3.lock",
            "pkg:composer/humbug/box@4.2.0",
            "pkg:composer/amphp/parallel-functions@v1.1.0",
            "pkg:composer/amphp/parallel@v1.4.2",
          ],
          [
            "composer-3.lock",
            "pkg:composer/humbug/box@4.2.0",
            "pkg:composer/amphp/parallel-functions@v1.1.0",
            "pkg:composer/amphp/parallel@v1.4.2",
            "pkg:composer/amphp/byte-stream@v1.8.1",
          ],
          [
            "composer-3.lock",
            "pkg:composer/humbug/box@4.2.0",
            "pkg:composer/amphp/parallel-functions@v1.1.0",
            "pkg:composer/amphp/parallel@v1.4.2",
            "pkg:composer/amphp/process@v1.1.4",
          ],
          [
            "composer-3.lock",
            "pkg:composer/humbug/box@4.2.0",
            "pkg:composer/amphp/parallel-functions@v1.1.0",
            "pkg:composer/amphp/parallel@v1.4.2",
            "pkg:composer/amphp/sync@v1.4.2",
          ],
        ],
      },
      {
  ...
  assert.deepStrictEqual(retMap.pkgList[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: [["composer-4.lock", "pkg:composer/gwtphp/gwtphp@dev-master"]],
      },
    ],
    evidence: {
      identity: {
...
it("parseComposerLock populates SrcFile multi-source dependency chains", () => {
  const lockFile = "./test/data/composer-multi-source.lock";
  const ret = parseComposerLock(lockFile, {
    "acme/app": "^1.0",
    "acme/dev-tool": "^0.9",
  });
  // Convenience lookup
  const byName = Object.fromEntries(ret.pkgList.map((p) => [p.name, p]));
  const getChain = (name) => {
    const sf = byName[name]?.properties?.find((p) => p.name === "SrcFile");
    return sf?.value;
  };
  // Every package must have an Array<Array<string>> SrcFile whose first
  // element is the lock file (relative to its dir).
  for (const pkg of ret.pkgList) {
    const sf = pkg.properties?.find((p) => p.name === "SrcFile");
    assert.ok(sf, `expected SrcFile for ${pkg.name}`);
    assert.ok(Array.isArray(sf.value), `${pkg.name} SrcFile must be array`);
    assert.ok(
      Array.isArray(sf.value[0]),
      `${pkg.name} SrcFile must be array of arrays`,
    );
    for (const chain of sf.value) {
      assert.deepStrictEqual(chain[0], "composer-multi-source.lock");
      assert.ok(
        !chain.includes(pkg["bom-ref"]),
        `chain for ${pkg.name} must not include its own bom-ref`,
      );
    }
  }
  // app is a true root: single chain containing only the lock file.
  assert.deepStrictEqual(getChain("app"), [["composer-multi-source.lock"]]);
  // feature-a / feature-b: single path through app.
  assert.deepStrictEqual(getChain("feature-a"), [
    ["composer-multi-source.lock", "pkg:composer/acme/app@1.0.0"],
  ]);
  assert.deepStrictEqual(getChain("feature-b"), [
    ["composer-multi-source.lock", "pkg:composer/acme/app@1.0.0"],
  ]);
  // shared-core is reached via THREE distinct paths
  // (feature-a, feature-b, dev-tool).
  const sharedCore = getChain("shared-core");
  assert.deepStrictEqual(sharedCore.length, 3);
  const sharedKeys = new Set(sharedCore.map((c) => c.join("|")));
  assert.ok(
    sharedKeys.has(
      "composer-multi-source.lock|pkg:composer/acme/app@1.0.0|pkg:composer/acme/feature-a@1.2.0",
    ),
  );
  assert.ok(
    sharedKeys.has(
      "composer-multi-source.lock|pkg:composer/acme/app@1.0.0|pkg:composer/acme/feature-b@1.3.0",
    ),
  );
  assert.ok(
    sharedKeys.has(
      "composer-multi-source.lock|pkg:composer/acme/dev-tool@0.9.0",
    ),
  );
  // low-level-util chains carry shared-core as their tail in every path.
  const lowLevel = getChain("low-level-util");
  assert.deepStrictEqual(lowLevel.length, 3);
  for (const chain of lowLevel) {
    assert.deepStrictEqual(
      chain[chain.length - 1],
      "pkg:composer/acme/shared-core@2.5.1",
    );
  }
  // self-referencing must still be treated as a root despite the self loop
  // — proves the buildDependencyChainsForward self-dep fix is in effect.
  assert.deepStrictEqual(getChain("self-referencing"), [
    ["composer-multi-source.lock"],
  ]);
  // dev-tool comes from packages-dev and is not depended on by anyone.
  assert.deepStrictEqual(getChain("dev-tool"), [
    ["composer-multi-source.lock"],
  ]);
  // dev-only packages keep the optional scope, but still get chains.
  assert.deepStrictEqual(byName["dev-tool"].scope, "optional");
});
...
it("parseGemfileLockData", async () => {
  ...
  // Check first package properties
  let firstPkg = retMap.pkgList[0];
  assert.deepStrictEqual(firstPkg.name, "actioncable");
  assert.deepStrictEqual(firstPkg.version, "6.0.0");
...
it("parseGemfileLockData SrcFile chain tracking", async () => {
  const projectRoot = "./test/data";
  const gemfileLockFile = "./test/data/Gemfile1.lock";
  
  const gemLockData = readFileSync(gemfileLockFile, { encoding: "utf-8" });
  const result = await parseGemfileLockData(gemLockData, gemfileLockFile, projectRoot);
  const { pkgList, dependenciesList } = result;
  ...
  assert.ok(Array.isArray(directDepSrcFile.value), "SrcFile should be array");
  assert.ok(Array.isArray(directDepSrcFile.value[0]), "SrcFile should be array of arrays");
  assert.deepStrictEqual(directDepSrcFile.value[0][0], "Gemfile1.lock", "Direct dep chain should be just lock file");
  ...
  const transitiveSrcFile = transitiveDep.properties.find(p => p.name === "SrcFile");
  assert.ok(transitiveSrcFile.value[0].length >= 2, "Transitive should have lock file + parent(s)");
  assert.deepStrictEqual(transitiveSrcFile.value[0][0], "Gemfile1.lock", "Chain should start with lock file");
  assert.ok(transitiveSrcFile.value[0][1].startsWith("pkg:"), "Second element should be parent PURL");
  ...
  // Test packages with multiple paths
  const multiPathPkg = pkgList.find(p => {
    const srcFile = p.properties?.find(pr => pr.name === "SrcFile");
    return srcFile && srcFile.value.length > 1;
  });
  if (multiPathPkg) {
    ...
    // All chains should start with lock file
    for (const chain of multiPathSrcFile.value) {
      assert.deepStrictEqual(chain[0], "Gemfile1.lock", "All chains should start with lock file");
    }
  }
});
...
describe("yarn 1 alias resolution from resolved URL", () => {
  it("should extract real package name from resolved URL with scoped packages", async () => {
    const result = await parseYarnLock("./test/data/yarn_locks/yarn-alias.lock");

    const pkgMap = new Map();
    for (const pkg of result.pkgList) {
      const fullName = pkg.group ? `${pkg.group}/${pkg.name}` : pkg.name;
      if (!pkgMap.has(fullName)) {
        pkgMap.set(fullName, []);
      }
      pkgMap.get(fullName).push(pkg.version);
    }

    // Unscoped overriden by scoped
    // "babel-helper-transform-fixture-test-runner": "npm:@babel/helper-transform-fixture-test-runner@^7.28.6"
    assert.ok(!pkgMap.has("babel-helper-transform-fixture-test-runner"), 
      "babel-helper-transform-fixture-test-runner should not exist");
    assert.deepStrictEqual(pkgMap.get("@babel/helper-transform-fixture-test-runner"), ["7.28.6"],
      "@babel/helper-transform-fixture-test-runner should be 7.28.6");

    // Scoped overriden by scoped and manipulated
    //"@babel/register": "npm:@babel/helper-validator-identifier@*"
    assert.ok(!pkgMap.has("@babel/register"),
      "@babel/register should not exist");
    assert.deepStrictEqual(pkgMap.get("@babel/helper-validator-identifier").sort(), ["7.28.5", "7.9.5"],
      "@babel/helper-validator-identifier should have versions 7.28.5 and 7.9.5");

    // Scoped manipulated
    assert.ok(!pkgMap.has("@types/is"),
      "@types/is should not exist");
    assert.deepStrictEqual(pkgMap.get("@types/abs"), ["1.3.4"],
      "@types/abs should be 1.3.4");

    // Scoped overriden by unscoped
    // "@babel/code-frame": "npm:babel-code-frame@~6.26.0"
    assert.ok(!pkgMap.has("@babel/code-frame"),
      "@babel/code-frame should not exist");
    assert.deepStrictEqual(pkgMap.get("babel-code-frame"), ["6.26.0"],
      "babel-code-frame should be 6.26.0");

    // Version overriden
    // "@babel/compat-data": "~7.9.0"
    assert.deepStrictEqual(pkgMap.get("@babel/compat-data"), ["7.9.6"],
      "@babel/compat-data should be 7.9.6");

    // Overriden and manipulated
    // "@babel/template": "npm:@babel/parser@^7"
    assert.ok(!pkgMap.has("@babel/template"),
      "@babel/template should not exist");
    assert.deepStrictEqual(pkgMap.get("@babel/parser").sort(), ["7.29.0", "7.9.6"],
      "@babel/parser should have versions 7.29.0 and 7.9.6");

    // Unscoped overriden by unscoped
    // "ansi-styles": "npm:ansi-regex@*"
    assert.ok(!pkgMap.has("ansi-styles"),
      "ansi-styles should not exist");
    assert.deepStrictEqual(pkgMap.get("ansi-regex").sort(), ["2.1.1", "6.2.2"],
      "ansi-regex should have versions 2.1.1 and 6.2.2");
  });
});

describe("berry alias resolution from resolution field", () => {
  it("should extract real package name from resolution field with scoped packages", async () => {
    const result = await parseYarnLock("./test/data/yarn_locks/berry-alias.lock");
...
describe("multiple fragments without resolved name", () => {
  it("should create dependency entries for ALL packages in a multi-fragment lockfile entry", async () => {
    const result = await parseYarnLock("./test/data/yarn_locks/yarn-multiple-fragments.lock");

    // Find pkg-a and pkg-b in pkgList
    const pkgA = result.pkgList.find(pkg => pkg.name === "pkg-a");
    const pkgB = result.pkgList.find(pkg => pkg.name === "pkg-b");
    const lodash = result.pkgList.find(pkg => pkg.name === "lodash");

    assert.ok(pkgA, "pkg-a should exist in pkgList");
    assert.ok(pkgB, "pkg-b should exist in pkgList");
    assert.ok(lodash, "lodash should exist in pkgList");

    // Find dependency entries for both packages
    const pkgADeps = result.dependenciesList.find(dep => dep.ref === pkgA["bom-ref"]);
    const pkgBDeps = result.dependenciesList.find(dep => dep.ref === pkgB["bom-ref"]);

    // Both pkg-a and pkg-b should have lodash as a dependency
    // This tests the fix for the bug where only the last package in a multi-fragment
    // lockfile entry would get dependency information
    assert.ok(pkgADeps, "pkg-a should have a dependency entry");
    assert.ok(pkgBDeps, "pkg-b should have a dependency entry");
    
    assert.ok(
      pkgADeps.dependsOn.includes(lodash["bom-ref"]),
      "pkg-a should depend on lodash"
    );
    assert.ok(
      pkgBDeps.dependsOn.includes(lodash["bom-ref"]),
      "pkg-b should depend on lodash"
    );
  });
});
...
it("parse requirements.txt", async () => {
  ...
  assert.deepStrictEqual(deps[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: [["requirements.freeze.txt"]], // Relative path after normalization
      },
    ],
    evidence: {
      identity: {
  ...
  assert.deepStrictEqual(deps[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: [["chen-science-requirements.txt"]], // Relative path after normalization
      },
      {
  ...
  assert.deepStrictEqual(deps[0], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: [["requirements-lock.linux_py3.txt"]], // Relative path after normalization
      },
    ],
    hashes: [
      {
...
it("parsePyLockData populates SrcFile dependency chains", async () => {
  const retMap = await parsePyLockData(
    readFileSync("./test/data/poetry.lock", { encoding: "utf-8" }),
    "./test/data/poetry.lock",
  );
  // Every package should have a SrcFile property as Array<Array<string>> where
  // the first element is the lock file path (relative to its directory).
  for (const pkg of retMap.pkgList) {
    const srcFile = pkg.properties?.find((p) => p.name === "SrcFile");
    assert.ok(srcFile, `expected SrcFile for ${pkg.name}`);
    assert.ok(
      Array.isArray(srcFile.value),
      `expected SrcFile to be array for ${pkg.name}`,
    );
    assert.ok(
      Array.isArray(srcFile.value[0]),
      `expected SrcFile to be Array<Array<string>> for ${pkg.name}`,
    );
    assert.deepStrictEqual(srcFile.value[0][0], "poetry.lock");
    // The package's own bom-ref should never appear in any of its chains.
    for (const chain of srcFile.value) {
      assert.ok(
        !chain.includes(pkg["bom-ref"]),
        `chain for ${pkg.name} should not include its own bom-ref`,
      );
    }
  }
  // At least one transitive dep should carry parent PURLs in its chain.
  const atomicwrites = retMap.pkgList.find((p) => p.name === "atomicwrites");
  assert.ok(atomicwrites, "atomicwrites should be present in poetry.lock");
  const aSrc = atomicwrites.properties.find((p) => p.name === "SrcFile");
  const hasMultiSegmentChain = aSrc.value.some(
    (chain) =>
      chain.length > 1 && chain.slice(1).every((s) => s.startsWith("pkg:pypi/")),
  );
  assert.ok(
    hasMultiSegmentChain,
    "atomicwrites should have at least one chain with parent PURLs",
  );

  // pdm.lock contains a self-dependency on pdm itself; chain building must
  // still treat pdm as a root so its descendants (e.g. blinker) get chains.
  const pdmRet = await parsePyLockData(
    readFileSync("./test/data/pdm.lock", { encoding: "utf-8" }),
    "./test/data/pdm.lock",
  );
  const blinker = pdmRet.pkgList.find((p) => p.name === "blinker");
  assert.ok(blinker, "blinker should be present in pdm.lock");
  const bSrc = blinker.properties.find((p) => p.name === "SrcFile");
  assert.ok(Array.isArray(bSrc?.value), "blinker SrcFile should be a chain");
  assert.ok(
    bSrc.value.some(
      (chain) =>
        chain[0] === "pdm.lock" &&
        chain.includes("pkg:pypi/pdm@2.7.4") &&
        !chain.includes(blinker["bom-ref"]),
    ),
    "blinker chain should descend from pdm without including itself",
  );
});
...
it("parse scala sbt tree with srcChain", () => {
  const retMap = parseSbtTree("./test/data/atom-sbt-tree.txt");
  
  assert.deepStrictEqual(retMap.pkgList.length, 153);
  
  // Find a direct dependency (level 0) - better-files
  const directDep = retMap.pkgList.find(pkg => 
    pkg.name === "better-files_3" && pkg.version === "3.9.2"
  );
  assert.ok(directDep, "Should find better-files_3");
  assert.ok(directDep.properties, "Should have properties");
  const directSrcFile = directDep.properties.find(p => p.name === "SrcFile");
  assert.ok(directSrcFile, "Should have SrcFile property");
  assert.ok(Array.isArray(directSrcFile.value), "SrcFile should be array");
  assert.ok(Array.isArray(directSrcFile.value[0]), "SrcFile should be array of arrays");
  assert.deepStrictEqual(directSrcFile.value[0].length, 2, "Direct dep chain should have 2 elements: file + root (not including itself)");
  assert.deepStrictEqual(directSrcFile.value[0][0], "atom-sbt-tree.txt", "Should be relative path");
  assert.ok(directSrcFile.value[0][1].includes("atom_3") && directSrcFile.value[0][1].includes("1.0.0"), "Should include root project");
  // Verify package itself is NOT in chain
  assert.ok(!directSrcFile.value[0].some(item => item.includes("better-files_3:3.9.2")), "Package should NOT be in its own chain");
  
  // Find a transitive dependency - scala3-library_3 (not evicted)
  const transDep = retMap.pkgList.find(pkg =>
    pkg.name === "scala3-library_3" && pkg.version === "3.3.0"
  );
  assert.ok(transDep, "Should find scala3-library_3 3.3.0 (transitive)");
  const transSrcFile = transDep.properties.find(p => p.name === "SrcFile");
  assert.ok(transSrcFile, "Should have SrcFile property");
  assert.ok(Array.isArray(transSrcFile.value[0]), "Should be array of arrays");
  assert.ok(transSrcFile.value[0].length >= 2, "Transitive chain should have at least 2 elements");
  assert.deepStrictEqual(transSrcFile.value[0][0], "atom-sbt-tree.txt", "Should start with sbt tree file (relative)");
  // Verify package itself is NOT in chain
  assert.ok(!transSrcFile.value[0].some(item => item.includes("scala3-library_3:3.3.0")), "Package should NOT be in its own chain");
});
...
it("parse bazel skyframe", () => {
  const deps = parseBazelSkyframe(
    ...
    "./test/data/bazel/bazel-state.txt",
  );
...
it("parse swift deps files", () => {
  ...
  assert.deepStrictEqual(retData.rootList[0], {
    ...
    properties: [
      ...
      { name: "SrcFile", value: [["swift-deps.json"]] },
    ],
  ...
  assert.deepStrictEqual(retData.pkgList[1], {
    ...
    properties: [
      ...
      },
      {
        name: "SrcFile",
        value: [
          ["swift-deps.json", "pkg:swift/swift-markdown@unspecified"],
        ],
      },
    ],
  ...
  assert.deepStrictEqual(retData.pkgList[1], {
    ...
    properties: [
      {
        name: "SrcFile",
        value: [["swift-deps.json", "pkg:swift/swift-certificates@unspecified"]],
      },
    ],
  });
  ...
  // Multi-source chains for Swift Package tree (diamond + dev tool)
  const multi = parseSwiftJsonTree(
    readFileSync("./test/data/swift-deps-multi-source.json", {
      encoding: "utf-8",
    }),
    "./test/data/swift-deps-multi-source.json",
  );
  const chainFor = (pkgName) => {
    const p = multi.pkgList.find((q) => q.name === pkgName);
    return p?.properties?.find((q) => q.name === "SrcFile")?.value;
  };
  for (const pkg of multi.pkgList) {
    const sf = pkg.properties?.find((p) => p.name === "SrcFile");
    assert.ok(sf, `${pkg.name} should have SrcFile`);
    assert.ok(Array.isArray(sf.value));
    assert.ok(Array.isArray(sf.value[0]));
    for (const c of sf.value) {
      assert.deepStrictEqual(c[0], "swift-deps-multi-source.json");
      assert.ok(!c.includes(pkg["bom-ref"]));
    }
  }
  assert.deepStrictEqual(chainFor("sample-app"), [
    ["swift-deps-multi-source.json"],
  ]);
  const sharedCore = chainFor("shared-core");
  assert.deepStrictEqual(sharedCore.length, 3);
  const sharedKeys = new Set(sharedCore.map((c) => c.join("|")));
  for (const mid of [
    "pkg:swift/github.com/acme/feature-a@1.2.0",
    "pkg:swift/github.com/acme/feature-b@1.3.0",
    "pkg:swift/github.com/acme/dev-tool@0.9.0",
  ]) {
    assert.ok(
      sharedKeys.has(
        `swift-deps-multi-source.json|pkg:swift/sample-app@1.0.0|${mid}`,
      ),
      `expected route through ${mid}`,
    );
  }
  const lowLevel = chainFor("low-level-util");
  assert.deepStrictEqual(lowLevel.length, 3);
  for (const c of lowLevel) {
    assert.deepStrictEqual(
      c[c.length - 1],
      "pkg:swift/github.com/acme/shared-core@2.5.1",
    );
  }
...
it("parses minified js with version containing extra info (#selectize)", async () => {
  const file = "temp.min.js";
  const content = `/*! selectize.js 0.11.2 | https://github.com/brianreavis/selectize.js | Apache License */
(function(){console.log("test")})();`;
  ...
  assert.equal(result.length, 1);
  assert.equal(result[0].name, "selectize.js");
  assert.equal(result[0].version, "0.11.2");

  if (existsSync(file)) unlinkSync(file);
});

describe("minified file pattern matching", () => {
  const testDir = path.join("test", "data", "minified-pattern-test");

  it("should setup test fixtures for minified file patterns", () => {
    ...
    // Create files with different minification patterns
    writeFileSync(path.join(testDir, "jquery.min.js"), "// jquery.min.js");
    writeFileSync(path.join(testDir, "bootstrap-min.js"), "// bootstrap-min.js");
    writeFileSync(path.join(testDir, "lodash_min.js"), "// lodash_min.js");
    writeFileSync(path.join(testDir, "regular.js"), "// regular.js");
    writeFileSync(path.join(testDir, "notmin.js"), "// notmin.js");

    assert.ok(existsSync(testDir));
  });

  it("should match .min.js pattern", () => {
    const files = getAllFiles(testDir, "**/*.min.js", {});
    assert.equal(files.length, 1);
    assert.ok(files[0].includes("jquery.min.js"));
  });

  it("should match all minified patterns with *[._-]min.js", () => {
    const files = getAllFiles(testDir, "**/*[._-]min.js", {});
    // Should find 3 files: jquery.min.js, bootstrap-min.js, lodash_min.js
  ...
  it("should not match non-minified files", () => {
    const files = getAllFiles(testDir, "**/*[._-]min.js", {});
    const basenames = files.map((f) => path.basename(f));

    // Should NOT include regular.js or notmin.js
    assert.ok(!basenames.includes("regular.js"));
    assert.ok(!basenames.includes("notmin.js"));
  });

  it("should cleanup test fixtures", () => {
    if (existsSync(testDir)) {
      rmSync(testDir, { recursive: true, force: true });
    }
    assert.ok(!existsSync(testDir));
...
describe("getAllFiles with symlink handling", () => {
  const testDir = path.join("test", "data", "symlink-test");
  const realDir = path.join(testDir, "real");
  const symlinkDir = path.join(testDir, "symlinks");
  const file1 = path.join(realDir, "test1.js");
  const file2 = path.join(realDir, "test2.js");
  const symlink1 = path.join(symlinkDir, "test1-link.js");
  const symlink2 = path.join(symlinkDir, "test2-link.js");

  it("should setup test fixtures", () => {
    if (existsSync(testDir)) {
      rmSync(testDir, { recursive: true, force: true });
    }
    ...
    writeFileSync(file1, "console.log('test1');");
    writeFileSync(file2, "console.log('test2');");

    symlinkSync(path.resolve(file1), symlink1);
    symlinkSync(path.resolve(file2), symlink2);

    assert.ok(existsSync(file1));
    assert.ok(existsSync(file2));
    assert.ok(existsSync(symlink1));
    assert.ok(existsSync(symlink2));
  });

  it("should find both real files and symlinks without resolveSymlinks option", () => {
    const files = getAllFiles(testDir, "**/*.js", {});
    ...
    const symlinksFound = files.filter((f) => f.includes("symlinks"));
    const realFound = files.filter((f) => f.includes("real"));

    assert.equal(symlinksFound.length, 2);
    assert.equal(realFound.length, 2);
  });

  it("should return duplicates when resolveSymlinks is explicitly false", () => {
    const files = getAllFiles(testDir, "**/*.js", { resolveSymlinks: false });
    assert.equal(files.length, 4);
  ...
  it("should deduplicate symlinks when resolveSymlinks option is true", () => {
    const files = getAllFiles(testDir, "**/*.js", { resolveSymlinks: true });
    assert.equal(files.length, 2);

    const fileBasenames = files.map((f) => path.basename(f)).sort();
    assert.deepStrictEqual(fileBasenames, ["test1.js", "test2.js"]);

    files.forEach((f) => {
      assert.ok(f.includes("/real/"));
      assert.ok(!f.includes("/symlinks/"));
    });
  });

  it("should return duplicates in getAllFilesWithIgnore by default", () => {
    const files = getAllFilesWithIgnore(testDir, "**/*.js", false, []);
    assert.equal(files.length, 4);
  });

  it("should return duplicates in getAllFilesWithIgnore when resolveSymlinks is false", () => {
    const files = getAllFilesWithIgnore(
      testDir,
      "**/*.js",
      false,
      [],
  ...
  it("should deduplicate in getAllFilesWithIgnore when resolveSymlinks is true", () => {
    const files = getAllFilesWithIgnore(
      testDir,
      "**/*.js",
      false,
...
describe("Clojure occurrence tests", async () => {
  const { addClojureOccurrences } = await import("./utils.js");

  it("addClojureOccurrences should match exact package names", () => {
    const pkgList = [
      {
        name: "cheshire",
        version: "5.10.0",
        purl: "pkg:clojars/cheshire@5.10.0"
      },
      {
        group: "org.clojure",
        name: "data.json",
        version: "2.4.0",
        purl: "pkg:clojars/org.clojure/data.json@2.4.0"
      }
    ];

    const clojureImports = {
      "cheshire": new Set(["src/myapp/core.clj"]),
      "org.clojure.data.json": new Set(["src/myapp/utils.clj"])
    };

    const result = addClojureOccurrences(pkgList, clojureImports);
    
    assert.ok(result[0].evidence?.occurrences);
    assert.equal(result[0].evidence.occurrences.length, 1);
    assert.equal(result[0].evidence.occurrences[0].location, "src/myapp/core.clj");
    assert.equal(result[0].scope, "required");

    assert.ok(result[1].evidence?.occurrences);
    assert.equal(result[1].evidence.occurrences.length, 1);
    assert.equal(result[1].evidence.occurrences[0].location, "src/myapp/utils.clj");
    assert.equal(result[1].scope, "required");
  });

  it("addClojureOccurrences should handle dash/underscore variations", () => {
    const pkgList = [
      {
        name: "ring-core",
        version: "1.9.0",
        purl: "pkg:clojars/ring-core@1.9.0"
      }
    ];

    const clojureImports = {
      "ring_core": new Set(["src/myapp/middleware.clj"])
    };
    ...
    assert.ok(result[0].evidence?.occurrences);
    assert.equal(result[0].evidence.occurrences.length, 1);
    assert.equal(result[0].evidence.occurrences[0].location, "src/myapp/middleware.clj");
  });

  it("addClojureOccurrences should match artifact name suffix", () => {
    const pkgList = [
      {
        group: "compojure",
        name: "core",
        version: "1.6.2",
        purl: "pkg:clojars/compojure/core@1.6.2"
      }
    ];

    const clojureImports = {
      "compojure.core": new Set(["src/myapp/routes.clj"])
    };
    ...
    assert.ok(result[0].evidence?.occurrences);
    assert.equal(result[0].evidence.occurrences.length, 1);
    assert.equal(result[0].evidence.occurrences[0].location, "src/myapp/routes.clj");
  });

  it("addClojureOccurrences should deduplicate locations", () => {
    const pkgList = [
      {
        name: "clj-http",
        version: "3.12.0",
        purl: "pkg:clojars/clj-http@3.12.0"
      }
    ];

    const clojureImports = {
      "clj-http": new Set(["src/myapp/api.clj", "src/myapp/client.clj", "src/myapp/api.clj"])
    };

    const result = addClojureOccurrences(pkgList, clojureImports);
    
    assert.ok(result[0].evidence?.occurrences);
    assert.equal(result[0].evidence.occurrences.length, 2);
    const locations = result[0].evidence.occurrences.map(o => o.location).sort();
    assert.deepStrictEqual(locations, ["src/myapp/api.clj", "src/myapp/client.clj"]);
  });

  it("addClojureOccurrences should not match unrelated packages", () => {
    const pkgList = [
      {
        name: "something-else",
        version: "1.0.0",
        purl: "pkg:clojars/something-else@1.0.0"
      }
    ];
