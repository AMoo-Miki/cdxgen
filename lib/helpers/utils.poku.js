...
it("parse gradle dependencies", async () => {
  ...
  // Chain should include the dependency path: org.web3j:core -> org.web3j:abi -> org.web3j:utils -> bcprov-jdk15on
  assert.ok(firstChain.some(v => v.includes("org.web3j:core")), "Chain should include parent dep");
  assert.ok(firstChain.some(v => v.includes("org.web3j:abi")), "Chain should include grandparent dep");
  assert.ok(firstChain.some(v => v.includes("org.web3j:utils")), "Chain should include great-grandparent dep");
    ...
    properties: [
      {
        ...
        value: [
          ...
          ["build.gradle", "testCompileClasspath"],
          [
            "build.gradle",
  ...
  assert.deepStrictEqual(parsedList.pkgList, [
    ...
    },
    {
      ...
      properties: [
        {
          name: "SrcFile",
          value: [["build.gradle", "io.appium:java-client@8.1.1"]],
        },
        {
          name: "cdx:gradle:GradleRootPath",
          value: "./test/data",
...
it("parse maven tree with srcChain", () => {
  ...
  // Test direct dependency - should have chain: [pom, scope, root-project, direct-dep]
  ...
  assert.deepStrictEqual(srcFileProp.value[0].length >= 3, true, "Direct dep chain should have at least 3 elements");
  assert.deepStrictEqual(srcFileProp.value[0][0], "pom.xml", "Should be relative path");
  assert.ok(srcFileProp.value[0][srcFileProp.value[0].length - 1].includes("jackson-databind:2.9.2"), "Should end with jackson-databind");
  
  // Test transitive dependency - should have longer chain
  ...
  assert.ok(transSrcFileProp.value[0].join(',').includes("jackson-databind"), "Should include parent dependency (jackson-databind)");
  assert.ok(transSrcFileProp.value[0][transSrcFileProp.value[0].length - 1].includes("jackson-annotations"), "Should end with itself");
  assert.ok(transSrcFileProp.value[0].length > jacksonDatabind.properties.find(p => p.name === "SrcFile").value[0].length, "Transitive chain should be longer than direct dep chain");
});
...
it("parse scala sbt tree with srcChain", () => {
  ...
  assert.ok(Array.isArray(directSrcFile.value[0]), "SrcFile should be array of arrays");
  assert.deepStrictEqual(directSrcFile.value[0].length, 3, "Direct dep chain should have 3 elements: file + root + dep");
  assert.deepStrictEqual(directSrcFile.value[0][0], "atom-sbt-tree.txt", "Should be relative path");
  assert.ok(directSrcFile.value[0][1].includes("atom_3:1.0.0"), "Should include root project");
  assert.ok(directSrcFile.value[0][2].includes("better-files_3:3.9.2"), "Should end with better-files");
  ...
  assert.ok(transSrcFile.value[0][transSrcFile.value[0].length - 1].includes("scala3-library_3:3.3.0"), "Should end with itself");
});
