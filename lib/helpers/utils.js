...
export async function parseGradleDep(
  ...
  // Helper function to get or create SrcFile property in a dep
  const getSrcFileProperty = (dep) => {
    if (!dep.properties) {
      dep.properties = [];
    }
      ...
      while ((match = depRegex.exec(rline))) {
        ...
        if (version !== undefined || group === "project") {
          ...
          if (depsMap.has(bomRef)) {
            // Dependency exists - add this chain to its SrcFile
            const existingDep = depsMap.get(bomRef);
            if (gradleRootPath) {
              const srcFileProp = getSrcFileProperty(existingDep);
              // Chain excludes current dep (it's redundant in its own SrcFile)
              const chain = [gradleRootPath, ...srcChain.slice(0, -1)];
              // Add chain if not duplicate
            ...
            if (gradleModules.has(name)) {
              ...
              // Add source file tracking - always as array of chains
              // Chain excludes current dep (redundant in its own SrcFile)
              if (gradleRootPath) {
                const chain = [gradleRootPath, ...srcChain.slice(0, -1)];
                adep.properties.push({
                  name: "SrcFile",
                  value: [chain], // Always array of arrays
                });
              }
...
export function runPigarInVenv(setupDir, tempVenvDir) {
  try {
    ...
    // Run pigar to generate requirements.txt
