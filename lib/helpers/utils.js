...
import yarnLockV1 from "@yarnpkg/lockfile";
...
export function getPythonCommand() {
  ...
  if (process.env.PYTHON_CMD) {
    ...
    try {
      const result = safeSpawnSync("python3", ["--version"], {
        encoding: "utf-8",
      });
      if (result.status === 0) {
        pythonCmd = "python3";
      ...
      // python3 not found, stick with python
    }
  }
  return pythonCmd;
}
 ...
 * @param {Boolean} resolveSymlinks whether to resolve symlinks and deduplicate
 *
 * @returns {Array[string]} List of matched files
 */
export function getAllFilesWithIgnore(
  ...
  try {
    ...
    if (filesLength > 1) {
      thoughtLog(
        `Found ${filesLength} files for the pattern '${pattern}' at '${dirPath}'.`,
      );
    }
    ...
    for (const file of files) {
      try {
        ...
        if (DEBUG_MODE) {
          console.warn(`Could not resolve symlink for ${file}:`, err.message);
        }
        resolvedPaths.add(file);
      }
    }

    if (resolvedPaths.size < filesLength) {
      thoughtLog(
        `Deduplicated ${filesLength} files to ${resolvedPaths.size} after resolving symlinks for '${pattern}' at '${dirPath}'.`,
      );
 ...
 */
export async function parsePkgLock(pkgLockFile, options = {}, projectRoot = null) {
  let pkgList = [];
  ...
  const parseArboristNode = (
    ...
    pkgSpecVersionCache = {},
    options = {},
    parentChain = [],  // Array tracking the chain from package.json to this node
  ) => {
    if (visited.has(node)) {
    ...
    if (node === rootNode) {
      ...
      // In npm lock files, aliased packages have a location like "node_modules/react-old"
      // where "react-old" is the alias but the actual package name is "react"
      let packageAlias = null;
      
      if (node.location) {
        const locationBasename = basename(node.location);
        // If the location basename differs from the package name, it's an alias
        if (locationBasename !== node.packageName) {
          packageAlias = locationBasename;
        }
      }
      
      // Build SrcFile chain: [pkgLockFile, parent1_purl, parent2_purl, ...]
      // The chain does NOT include the current package itself
      // Use full path for now - will be normalized to relative path later
      const srcFileChain = [pkgLockFile, ...parentChain];
      
      const properties = [
        {
          name: "SrcFile",
          value: [srcFileChain],  // Array of chains (could have multiple paths)
        },
      ];
      
      // Add alias property if present
      if (packageAlias) {
        properties.push({
    ...
    if (node.fsChildren && node.fsChildren.size > 0) {
      for (const workspaceNode of node.fsChildren) {
        // Build chain for workspace children: add current node's PURL to the chain
        const newChain = node === rootNode ? [] : [...parentChain, purlString];
        const {
    ...
    if (node !== rootNode && !node.edgesOut.size) {
      for (const child of node.children) {
        ...
        // Build chain for children: add current node's PURL to the chain
        const newChain = node === rootNode ? [] : [...parentChain, purlString];
        const {
          pkgList: childPkgList,
          dependenciesList: childDependenciesList,
    ...
    for (const edge of node.edgesOut.values()) {
      ...
      // Build chain for edge dependencies: add current node's PURL to the chain
      const newChain = node === rootNode ? [] : [...parentChain, purlString];
  ...
  ({ pkgList, dependenciesList } = parseArboristNode(
    ...
    pkgSpecVersionCache,
    options,
    [],  // Start with empty chain for root node
  ));

  // Normalize SrcFile paths to be relative to project root (if provided) or lock file directory
  const baseDir = projectRoot || dirname(pkgLockFile);
  normalizeSrcFilePaths(pkgList, baseDir);
...
export async function parseYarnLock(
  ...
  // Parse and normalize the lockfile into a common format
  let normalizedEntries;
  if (lockData.includes(' version: ') || lockData.includes('__metadata:')) {
    // Yarn v2+ (Berry) - uses SYML format (Yarn's YAML variant)
    const parsedData = parseSyml(lockData);
    const { __metadata, ...dependencies } = parsedData;
    if (__metadata?.version) {
      normalizedEntries = normalizeYarnLockV2(dependencies);
    }
  } else {
    // Yarn v1
    const { object: dependencies } = yarnLockV1.parse(lockData);
    normalizedEntries = normalizeYarnLockV1(dependencies);
  }
  ...
  // Build spec-to-resolved map
  const specToResolvedMap = buildSpecToResolvedMap(normalizedEntries);

  // Create packages
  pkgList = createPackagesFromNormalizedEntries(normalizedEntries, yarnLockFile);

  // Build dependencies map
  const dependenciesMap = buildDependenciesMap(normalizedEntries, pkgList);

  // Build dependencies list by resolving specs to actual package refs
  // Use specToResolvedMap to find exact versions for dependency specs
  for (const pkg of pkgList) {
    const deps = dependenciesMap.get(pkg["bom-ref"]);
    const dependsOn = [];
    
    if (deps) {
      for (const dep of deps) {
        // Build the spec key: "package-name@spec"
        const depFullName = dep.group ? `${dep.group}/${dep.name}` : dep.name;
        const specKey = `${depFullName}@${dep.spec}`;
        
        // Look up the resolved version from our spec map
        const resolved = specToResolvedMap.get(specKey);
        
        if (resolved) {
          // Found the exact resolved version!
          const resolvedPurl = new PackageURL(
            "npm",
          ...
          dependsOn.push(decodeURIComponent(resolvedPurl));
        }
      }
    }
    
    // Always create an entry, even if dependsOn is empty
    dependenciesList.push({
      ref: pkg["bom-ref"],
      ...
      if (!existingComponent) {
        try {
          ...
          try {
            ...
            thoughtLog(
              `Invalid workspace package PURL: ${workspacePkg}. Skipping.`,
            );
            continue;
          }

          if (purlObj && purlObj.name) {
            // Build lookup key for workspace files
            const wsLookupKey = purlObj.namespace 
            ...
            const workspaceComponent = {
              ...
              name: purlObj.name,
              version: purlObj.version || "0.0.0-use.local",
              purl: workspacePkg,
              "bom-ref": decodeURIComponent(workspacePkg),
              properties: [
                {
                  name: "SrcFile",
                  value: workspaceSrcFiles[wsLookupKey] || "package.json",
                },
                {
                ...
                },
                {
                  ...
                  value: workspaceSrcFiles[wsLookupKey] ? "true" : "false",
                },
              ],
              evidence: {
                identity: {
          ...
          thoughtLog(
            `Error processing workspace package "${workspacePkg}": ${e.message}`,
          );
        }
      }
    }
  ...
  // Add workspace references for yarn workspaces (after all packages are in pkgList)
  ...
  if (pkgList?.length) {
    pkgList = await yarnMetadata(pkgList, yarnLockFile);
  }

  // Build SrcFile dependency chains from dependenciesList
  if (pkgList?.length && dependenciesList?.length) {
    const yarnLockPath = yarnLockFile;
    
    // Use shared optimized BFS utility
    const chainsMap = buildDependencyChainsForward(dependenciesList, yarnLockPath);
    
    // Apply chains to packages
    for (const pkg of pkgList) {
      const chains = chainsMap.get(pkg["bom-ref"]);
      
      if (chains && chains.length > 0) {
        // Filter out any chains that contain the package itself (circular dependencies)
        const filteredChains = chains.filter(chain => !chain.includes(pkg["bom-ref"]));
        
        if (filteredChains.length > 0) {
          // Update or create SrcFile property
          const srcFileIdx = pkg.properties?.findIndex(p => p.name === "SrcFile");
          if (srcFileIdx !== undefined && srcFileIdx >= 0) {
            pkg.properties[srcFileIdx].value = filteredChains;
          } else {
            if (!pkg.properties) {
              pkg.properties = [];
            }
    ...
    // Always normalize paths with fallback for consistency
    const baseDir = projectRoot || dirname(yarnLockFile);
    normalizeSrcFilePaths(pkgList, baseDir);
  }
...
// Normalize Yarn v1 lockfile into common format
const normalizeYarnLockV1 = (dependencies) => {
  const entries = [];
  const reResolved = /^https:\/\/registry\.(?:yarnpkg\.com|npmjs\.org)\/(.+?)\/-\/(?:.+?)-(\d+\..+?)\.tgz#(.+)$/;
  ...
  const parseFragment = (fragment) => {
    // Handle Berry-style aliases in Yarn v1 lockfiles: "alias@npm:actual-package@version"
    if (fragment.includes('@npm:')) {
      // Extract the actual package name from after @npm:
      const npmPart = fragment.split('@npm:')[1];
      // Remove the version spec
      return npmPart.replace(/@[^@]*$/, '');
    }
    // Standard format: remove the version spec
  ...
  for (const key in dependencies) {
    ...
    // Extract resolved package info (this is the same for all fragments)
    let resolvedGroup, resolvedName, resolvedVersion;
    if (match) {
      ({ group: resolvedGroup, name: resolvedName } = parseName(match[1]));
      resolvedVersion = match[2];
    } else {
      // For multi-fragment keys without resolved URL, we'll extract per fragment below
      resolvedVersion = dep.version;
    }

    // Extract integrity
    let integrity = dep.integrity;
    if (!integrity && match?.[3]) {
      integrity = `sha256-${match[3]}`;
    }

    // Split multi-fragment keys and create an entry for EACH fragment
    const fragments = key.split(/, /);
    for (const fragment of fragments) {
      let fragGroup, fragName;
      
      if (match) {
        // If we have a resolved URL, use that for package info (all fragments resolve to same package)
        fragGroup = resolvedGroup;
        fragName = resolvedName;
      } else {
        // No resolved URL - extract package name from this specific fragment
        const cleanFragment = parseFragment(fragment);
        ({ group: fragGroup, name: fragName } = parseName(cleanFragment));
      }
      
      if (!fragName || !resolvedVersion) {
        continue;
      }

      // Create a normalized entry for this fragment
      entries.push({
        key: fragment, // Use individual fragment as key for tracking
        version: dep.version,
        resolvedGroup: fragGroup || "",
        resolvedName: fragName,
        resolvedVersion,
        integrity,
...
// Normalize Yarn v2+ (Berry) lockfile into common format
const normalizeYarnLockV2 = (dependencies) => {
  const entries = [];
  ...
  const extractPatchInfo = (key, resolution) => {
    // Extract patch identifier from key or resolution
    // Patch formats:
    // 1. #builtin<compat/resolve>
    // 2. #~builtin<compat/resolve>
    // 3. #optional!builtin<compat/resolve>
    // 4. #~/.yarn/patches/ink-npm-3.0.8-hash.patch
    // 5. #./.yarn/patches/package.patch
    
    let patchId = null;
    
    // Try to extract from key first (most reliable)
    if (key.includes('@patch:') && key.includes('#')) {
      // For multi-fragment keys, extract from first fragment only
      const firstFragment = key.split(', ')[0];
      const hashPart = firstFragment.split('#')[1];
      if (hashPart) {
        // Extract the part before :: or end of string
        patchId = hashPart.split('::')[0];
      }
    } else if (resolution?.includes('@patch:') && resolution.includes('#')) {
      const hashPart = resolution.split('#')[1];
      if (hashPart) {
        patchId = hashPart.split('::')[0];
      }
    }
    ...
    if (patchId) {
      // Normalize patch ID:
      // 1. Remove optional! prefix
      patchId = patchId.replace(/^optional!/, '');
      
      // 2. Normalize builtin patches: ~builtin<compat/X> or builtin<compat/X>
      if (patchId.includes('builtin<')) {
        // Extract just the compat name: builtin<compat/resolve> -> resolve
        const match = patchId.match(/builtin<compat\/([^>]+)>/);
        if (match) {
          patchId = `builtin-${match[1]}`;
        }
      } else if (patchId.includes('.yarn/patches/')) {
        // Custom file patch: ~/.yarn/patches/ink-npm-3.0.8-hash.patch
        // Extract filename without path and extension
        const filename = patchId.split('/').pop().replace(/\.patch$/, '');
        patchId = `custom-${filename}`;
      }
      
      // 3. Normalize special characters to make it URL/PURL safe
      patchId = patchId
        .replace(/[<>]/g, '-')
        .replace(/[:/\\~.]/g, '-')
        .replace(/\s+/g, '-')
  ...
  for (const key of Object.keys(dependencies)) {
    ...
    // Check if this is a patch protocol entry
    const patchId = extractPatchInfo(key, dep.resolution);
    
    // Extract resolved package info
    let resolvedGroup, resolvedName, resolvedVersion;
    
    // For patch protocol, the resolution is URL-encoded
    let resolutionStr = dep.resolution || '';
    if (resolutionStr.includes('%3A')) {
      resolutionStr = decodeURIComponent(resolutionStr);
    }
    
    const parts = resolutionStr.split('@npm:');
    if (parts?.length === 2) {
      let packagePart = parts[0];
      // For patch protocol, clean up the package name: "resolve@patch:resolve" -> "resolve"
      if (packagePart.includes('@patch:')) {
        packagePart = packagePart.split('@patch:').pop();
      }
      ({ group: resolvedGroup, name: resolvedName } = parseName(packagePart));
      resolvedVersion = parts[1];
      
      // For patch protocol, extract the base version before the patch info
      if (resolvedVersion?.includes('#')) {
        resolvedVersion = resolvedVersion.split('#')[0];
      }
      // Remove any trailing metadata after ::
      if (resolvedVersion?.includes('::')) {
        resolvedVersion = resolvedVersion.split('::')[0];
      }
    } else if (resolutionStr.includes('@workspace:')) {
      const wsParts = resolutionStr.split('@workspace:');
      ({ group: resolvedGroup, name: resolvedName } = parseName(wsParts[0]));
      resolvedVersion = dep.version || "0.0.0-use.local";
    } else if (resolutionStr.includes('@patch:')) {
      // Handle patch without npm: protocol (shouldn't happen but be safe)
      const cleanKey = parseFragment(key.replace('@patch:', ''));
      ({ group: resolvedGroup, name: resolvedName } = parseName(cleanKey));
      resolvedVersion = dep.version;
    } else {
      const cleanKey = parseFragment(key);
      ({ group: resolvedGroup, name: resolvedName } = parseName(cleanKey));
      resolvedVersion = dep.version;
    }
    ...
    // For patch protocol, append patch identifier to version
    if (patchId) {
      resolvedVersion = `${resolvedVersion}-patch-${patchId}`;
    }

    // Extract integrity
    let integrity = dep.integrity;
    if (dep.checksum && !integrity) {
      try {
        let hexChecksum = dep.checksum;
        if (hexChecksum.includes('/')) {
          hexChecksum = hexChecksum.split("/")[1];
        }
        if (hexChecksum) {
          integrity = "sha512-" + Buffer.from(hexChecksum, "hex").toString("base64");
        }
      } catch (e) {
        // Ignore
      }
    ...
    const fragments = key.split(/, /);
    for (const fragment of fragments) {
      // Normalize entry for this fragment
      entries.push({
        key: fragment, // Use individual fragment as key for tracking
        version: dep.version,
...
// Build spec-to-resolved map from normalized entries
const buildSpecToResolvedMap = (normalizedEntries) => {
  const specToResolvedMap = new Map();

  for (const entry of normalizedEntries) {
    const fullName = entry.resolvedGroup ? `${entry.resolvedGroup}/${entry.resolvedName}` : entry.resolvedName;
    const fragment = entry.key; // entry.key is already a single fragment
    
    // Extract spec from fragment
    let spec;
    if (fragment.includes('@npm:')) {
      spec = fragment.split('@npm:')[1];
    } else if (fragment.includes('@workspace:')) {
      spec = fragment.split('@workspace:')[1];
    } else {
      spec = fragment.split('@').pop();
    }
    
    const specKey = `${fullName}@${spec}`;
    specToResolvedMap.set(specKey, {
      group: entry.resolvedGroup,
      name: entry.resolvedName,
      version: entry.resolvedVersion,
    });
  }

  return specToResolvedMap;
};

// Create packages from normalized entries
const createPackagesFromNormalizedEntries = (normalizedEntries, yarnLockFile) => {
  const pkgList = new Map();

  for (const entry of normalizedEntries) {
    const idx = `${entry.resolvedGroup}//${entry.resolvedName}//${entry.resolvedVersion}`;
    
    if (!pkgList.has(idx)) {
      const purlString = new PackageURL(
        "npm",
        entry.resolvedGroup || undefined,
        entry.resolvedName,
      ...
      // Extract alias from key if present (e.g., "pdfjs-dist-v4_9@npm:pdfjs-dist@4.9.124")
      let packageAlias = null;
      if (entry.key && entry.key.includes('@npm:')) {
        const parts = entry.key.split('@npm:');
        const aliasWithVersion = parts[0];
        // Remove version specifier to get just the alias name
        const alias = aliasWithVersion.replace(/^@/, '!').replace(/@[^@]*$/, '').replace(/^!/, '@');
        const actualName = entry.resolvedGroup ? `${entry.resolvedGroup}/${entry.resolvedName}` : entry.resolvedName;
        // Only store as alias if it's different from the actual package name
        if (alias !== actualName && alias !== entry.resolvedName) {
          packageAlias = alias;
        }
      }
      ...
      // Add alias property if present (using CycloneDX npm namespace convention)
      if (packageAlias) {
        properties.push({
          name: "cdx:npm:package:alias",
          value: packageAlias,
...
// Build dependencies map from normalized entries
const buildDependenciesMap = (normalizedEntries, pkgList) => {
  const dependenciesMap = new Map();

  for (const entry of normalizedEntries) {
    // Find the package for this entry
    const pkg = pkgList.find(p => 
      p.group === entry.resolvedGroup && 
      p.name === entry.resolvedName && 
      p.version === entry.resolvedVersion
    );
    
    if (pkg && (entry.dependencies || entry.optionalDependencies)) {
      ...
      if (entry.dependencies) {
        for (const [depName, depSpec] of Object.entries(entry.dependencies)) {
          let depGroup = "";
          let depShortName = depName;
          
          // Parse scoped packages correctly: @babel/highlight -> group: @babel, name: highlight
          if (depName.startsWith("@") && depName.includes("/")) {
            const parts = depName.split("/");
            depGroup = parts[0];
            depShortName = parts.slice(1).join("/");
          }
          
          depSet.add({ name: depShortName, group: depGroup, spec: depSpec.replace(/^npm:/, "") });
        }
      }
      
      // Add optional dependencies
      if (entry.optionalDependencies) {
        for (const [depName, depSpec] of Object.entries(entry.optionalDependencies)) {
          let depGroup = "";
          let depShortName = depName;
      ...
      if (depSet.size > 0) {
        dependenciesMap.set(pkg["bom-ref"], depSet);
      }
    }
  }
...
/**
 * Yarn packages with metadata from local node_modules
 *
 ...
 */
export async function yarnMetadata(pkgList, lockFilePath) {
  ...
  if (DEBUG_MODE) {
    console.log(
      `Metadata for ${pkgList.length} yarn packages using local node_modules at ${nodeModulesDir}`,
    );
  }
  ...
  for (const pkg of pkgList) {
    ...
    // For Yarn, packages are typically stored as node_modules/package-name or node_modules/@scope/package-name
    const fullName = pkg.group ? `${pkg.group}/${pkg.name}` : pkg.name;
    const packagePath = join(nodeModulesDir, fullName);
...
export async function parsePnpmLock(
    ...
    if (yamlObj?.importers) {
      ...
      for (const rdk of Object.keys(rootDevDeps)) {
        ...
        if (specifier?.includes("npm:")) {
          ...
          // Also map the actual package bom-ref to the alias
          const versionValue = await getVersionNumPnpm(rootDevDeps[rdk]);
          if (versionValue) {
            const lastAtIndex = versionValue.lastIndexOf("@");
            if (lastAtIndex > 0) {
              const actualPkgName = versionValue.substring(0, lastAtIndex);
              const actualVersion = versionValue.substring(lastAtIndex + 1);
              const actualPurl = new PackageURL(
                "npm",
                "",
                actualPkgName,
                actualVersion,
              ...
              const actualBomRef = decodeURIComponent(actualPurl);
              possibleAliasesRefs[actualBomRef] = rdk;
            }
          }
        }
      ...
      for (const rdk of Object.keys({ ...rootOptionalDeps, ...rootPeerDeps })) {
        ...
        if (specifier?.includes("npm:")) {
          possibleAliases[rdk] = specifier;
          // Also map the actual package bom-ref to the alias
          const versionValue = await getVersionNumPnpm(rootOptionalDeps[rdk]);
          if (versionValue) {
            const lastAtIndex = versionValue.lastIndexOf("@");
            if (lastAtIndex > 0) {
              const actualPkgName = versionValue.substring(0, lastAtIndex);
      ...
      for (const dk of Object.keys(rootDirectDeps)) {
        if (DEBUG_MODE) {
          console.log(`[PNPM] Processing root dep: ${dk}, value:`, rootDirectDeps[dk]);
        }
        ...
        if (specifier?.includes("npm:")) {
          ...
          // Also map the actual package bom-ref to the alias
          // The version field in rootDirectDeps contains the resolved package reference
          // e.g., "@types/abs@1.3.4" or "babel-code-frame@6.26.0" or "react@17.0.2"
          const versionValue = await getVersionNumPnpm(rootDirectDeps[dk]);
          // versionValue could be "react@17.0.2" or "@types/abs@1.3.4" etc.
          if (versionValue) {
            const lastAtIndex = versionValue.lastIndexOf("@");
            if (lastAtIndex > 0) {
              const actualPkgName = versionValue.substring(0, lastAtIndex);
              ...
              const actualBomRef = decodeURIComponent(actualPurl);
              possibleAliasesRefs[actualBomRef] = dk;
              if (DEBUG_MODE) {
                console.log(`[PNPM Alias] Root direct: Mapped ${actualBomRef} -> ${dk} (versionValue: ${versionValue})`);
              }
            }
          }
        }
      ...
      for (const importedComponentName of Object.keys(yamlObj?.importers)) {
        ...
        if (fallbackMode) {
          // Only create fallback PURL if parentComponent exists
          if (parentComponent) {
            ...
            // Skip this workspace component if we can't determine its PURL
            continue;
          }
        }
  ...
  for (const apkg of pkgList) {
    ...
    if (possibleAliasesRefs[apkg["bom-ref"]]) {
      if (DEBUG_MODE) {
        console.log(`[PNPM Alias] Applying alias to ${apkg["bom-ref"]}: ${possibleAliasesRefs[apkg["bom-ref"]]}`);
      }
      apkg.properties.push({
        name: "cdx:npm:package:alias",
  ...
  // Build SrcFile dependency chains from dependenciesList
  if (pkgList?.length && dependenciesList?.length) {
    const pnpmLockPath = pnpmLock;
    // Use shared optimized BFS utility
    const chainsMap = buildDependencyChainsForward(dependenciesList, pnpmLockPath);
    // Apply chains to packages
    for (const pkg of pkgList) {
      const chains = chainsMap.get(pkg["bom-ref"]);
      if (chains && chains.length > 0) {
    ...
    // Always normalize paths with fallback for consistency
    const baseDir = projectRoot || dirname(pnpmLock);
    normalizeSrcFilePaths(pkgList, baseDir);
  }
...
export async function parseBowerJson(bowerJsonFile) {
  ...
  if (safeExistsSync(bowerJsonFile)) {
    try {
      ...
      // If no version in bower.json, try to find it from:
      // 1. .bower.json (generated by bower install with version metadata)
      // 2. package.json in the same directory
      if (!version) {
        const dirName = dirname(bowerJsonFile);
        const bowerMetaFile = join(dirName, ".bower.json");
        const packageJsonFile = join(dirName, "package.json");
        
        if (safeExistsSync(bowerMetaFile)) {
          try {
            const bowerMeta = JSON.parse(readFileSync(bowerMetaFile, "utf8"));
            version = bowerMeta.version || bowerMeta._release || "";
          } catch (_err) {
            // continue
          }
        } else if (safeExistsSync(packageJsonFile)) {
          try {
            const packageData = JSON.parse(readFileSync(packageJsonFile, "utf8"));
        ...
        if ((l.startsWith("/*!") || l.startsWith("  * ")) && l.length < 500) {
          ...
          if (tmpB && tmpB.length > 1) {
            ...
            if (pkgIdentifier.fullName !== "") {
              // Extract version, stopping at common delimiters like |, -, (
              let version = tmpB[1].replace(/^v/, "") || "";
              // Remove anything after | or - or ( to clean up version strings
 ...
 */
export function parseMavenTree(rawOutput, pomFile, projectRoot = null) {
  if (!rawOutput) {
    return {};
  }
  const depsMap = new Map(); // Use Map to track deps and allow chain updates
  const dependenciesList = [];
  const keys_cache = {};
  const level_trees = {};
  ...
  // Track dependency chain: [current_dep_purl, parent1_purl, parent2_purl, ...]
  // We build from root (empty) to leaves, then reverse for storage
  const chainStack = []; // Stack of PURLs representing current path in tree
  ...
  tmpA.forEach((l) => {
    ...
    if (tmpline?.length) {
      ...
      if (pkgArr && pkgArr.length > 2) {
        ...
        const bomRef = decodeURIComponent(purlString);
        const key = bomRef;
        const isRootProject = !first_ref; // First package is the root project
        if (!first_ref) {
          first_ref = bomRef;
        }
        
        // Update chainStack based on level changes
        if (level === 0) {
          // Direct dependency - no parents
          chainStack.length = 0;
        } else if (level > last_level) {
          // Going deeper - previous package is now a parent (unless it's the root project)
          if (last_purl && last_purl !== first_ref) {
            chainStack.push(last_purl);
          }
        } else if (level <= last_level) {
          // Same level or going back up - trim chain
          chainStack.length = level - 1; // level 1 has 0 parents, level 2 has 1 parent, etc.
        }
        ...
        if (!keys_cache[key]) {
          ...
          // Add SrcFile as array of arrays with pomFile + parent PURLs (excluding self)
          if (pomFile) {
            const chain = [pomFile, ...chainStack];
            properties.push({
              name: "SrcFile",
              value: [chain], // Store current resolution path
            });
          }
          ...
          depsMap.set(bomRef, apkg);
        } else {
          // Package already exists - add this chain as another resolution path
          const existingPkg = depsMap.get(bomRef);
          if (existingPkg && pomFile) {
            const srcFileProp = getSrcFileProperty(existingPkg);
            const newChain = [pomFile, ...chainStack];
            // Check if this chain is unique
            const chainStr = JSON.stringify(newChain);
            const isDuplicate = srcFileProp.value.some(
              existingChain => JSON.stringify(existingChain) === chainStr
            );
            if (!isDuplicate) {
              srcFileProp.value.push(newChain);
            }
          }
        }
  ...
  // Normalize SrcFile paths to be relative to projectRoot (or pomFile directory)
  if (pomFile) {
    const baseDir = projectRoot || dirname(pomFile);
    normalizeSrcFilePaths(deps, baseDir);
  }
 ...
 */
/**
 * Helper function to get or create SrcFile property in a component/dependency
 * Returns reference to the property object so it can be updated by reference
 * 
 * @param {Object} dep The dependency/component object
 * @returns {Object} Reference to the SrcFile property object
 */
export function getSrcFileProperty(dep) {
  if (!dep.properties) {
    dep.properties = [];
  }
  let srcFileProp = dep.properties.find(p => p.name === "SrcFile");
  if (!srcFileProp) {
    srcFileProp = { name: "SrcFile", value: [] };
    dep.properties.push(srcFileProp);
  }
  // Ensure value is always an array of chains
  if (!Array.isArray(srcFileProp.value)) {
    srcFileProp.value = [];
  }
  return srcFileProp;
}
...
/**
 * Post-process package list to make all SrcFile paths relative to basePath
 * Handles both array of arrays format and legacy string format
 * Normalizes both absolute and relative paths to be relative to basePath
 * 
 * @param {Array} pkgList List of packages
 * @param {string} basePath Base path to make paths relative to
 * @returns {Array} Modified package list with relative paths
 */
/**
 * Shared utility for building dependency chains using optimized BFS (forward propagation)
 * This builds chains from root packages downward to all descendants
 * 
 * @param {Array} dependenciesList - List of {ref, dependsOn} objects
 * @param {string} lockFileName - Name of the lock file for the first element of chains
 * @returns {Map} - Map of bom-ref to array of chains
 */
function buildDependencyChainsForward(dependenciesList, lockFileName) {
  // Build a map of package bom-ref to its direct dependencies,
  // stripping self-dependencies so they don't create artificial roots.
  const depsMap = new Map();
  for (const dep of dependenciesList) {
    const filtered = (dep.dependsOn || []).filter(
      (childRef) => childRef !== dep.ref,
    );
    depsMap.set(dep.ref, filtered);
  }

  // Find root packages (those not depended on by anyone other than themselves).
  // Self-references would otherwise hide an otherwise-true root.
  const allRefs = new Set(dependenciesList.map(d => d.ref));
  ...
  for (const dep of dependenciesList) {
    for (const childRef of dep.dependsOn || []) {
      ...
      dependedOn.add(childRef);
    }
  }
  
  // Build a map to store chains for each package
  const chainsMap = new Map();
  
  // Root packages are those not depended on by anyone
  const roots = [];
  for (const ref of allRefs) {
    if (!dependedOn.has(ref)) {
      roots.push(ref);
      // Root packages have just the lock file in their chain
      chainsMap.set(ref, [[lockFileName]]);
    }
  }
  
  // BFS to build chains from roots down to all descendants (optimized)
  const queue = [...roots];
  let queueIndex = 0; // Use index instead of shift() for O(1) performance
  const processed = new Set();
  
  // Pre-compute chain strings for efficient duplicate detection
  const chainStringCache = new Map();
  
  while (queueIndex < queue.length) {
    const currentRef = queue[queueIndex++];
    
    if (processed.has(currentRef)) {
      continue;
    }
    processed.add(currentRef);
    
    const currentChains = chainsMap.get(currentRef) || [[lockFileName]];
    const children = depsMap.get(currentRef) || [];
    
    for (const childRef of children) {
      // Build new chains for the child by extending parent chains
      const newChains = currentChains.map(chain => [...chain, currentRef]);
      
      // Merge with existing chains efficiently (package might be reached through multiple paths)
      const existingChains = chainsMap.get(childRef) || [];
      
      // Efficient duplicate detection using Set of chain strings
      const existingChainStrs = new Set();
      for (const existingChain of existingChains) {
        let str = chainStringCache.get(existingChain);
        if (!str) {
          str = JSON.stringify(existingChain);
          chainStringCache.set(existingChain, str);
        }
        existingChainStrs.add(str);
      }
      
      // Only add truly new chains
      const uniqueNewChains = [];
      for (const newChain of newChains) {
        let str = chainStringCache.get(newChain);
        if (!str) {
          str = JSON.stringify(newChain);
          chainStringCache.set(newChain, str);
        }
        if (!existingChainStrs.has(str)) {
          uniqueNewChains.push(newChain);
          existingChainStrs.add(str);
        }
      }
      
      if (uniqueNewChains.length > 0) {
        chainsMap.set(childRef, [...existingChains, ...uniqueNewChains]);
      }
      
      // Add child to queue if not already processed
      if (!processed.has(childRef)) {
        queue.push(childRef);
      }
    }
  }
...
/**
 * Shared utility for building dependency chains using optimized BFS (backward tracking)
 * This finds roots and tracks dependencies backward
 * 
 * @param {Map} pkgMap - Map of bom-ref to package objects
 * @param {Map} depsMap - Map of bom-ref to Set of dependency bom-refs
 * @param {string} lockFileName - Name of the lock file for the first element of chains
 * @returns {Map} - Map of bom-ref to array of chains
 */
function buildDependencyChains(pkgMap, depsMap, lockFileName) {
  // Identify root packages (those with no incoming edges)
  ...
  for (const bomRef of pkgMap.keys()) {
    if (!hasIncoming.has(bomRef)) {
      rootRefs.add(bomRef);
    }
  }
  
  // Build chains for each package using optimized BFS
  const chainsMap = new Map();
  
  // Pre-create chain string cache for deduplication
  const chainStrCache = new Map();
  
  for (const rootRef of rootRefs) {
    const queue = [{ ref: rootRef, chain: [] }];
    let queueIndex = 0; // Use index instead of shift() for O(1) performance
    const visited = new Set();
    
    while (queueIndex < queue.length) {
      const { ref, chain } = queue[queueIndex++];
      
      // Create unique key for this path
      const chainKey = `${ref}:${chain.join(",")}`;
      if (visited.has(chainKey)) {
        continue;
      }
      visited.add(chainKey);
      
      // Store chain for this package (excluding the package itself)
      if (!chainsMap.has(ref)) {
        chainsMap.set(ref, []);
      }
      
      const existingChains = chainsMap.get(ref);
      
      // Efficient duplicate detection using cached strings
      let chainStr = chainStrCache.get(chain);
      if (!chainStr) {
        chainStr = JSON.stringify(chain);
        chainStrCache.set(chain, chainStr);
      }
      
      // Check if this chain already exists
      let isDuplicate = false;
      for (const existingChain of existingChains) {
        let existingStr = chainStrCache.get(existingChain);
        if (!existingStr) {
          existingStr = JSON.stringify(existingChain);
          chainStrCache.set(existingChain, existingStr);
        }
        if (existingStr === chainStr) {
          isDuplicate = true;
          break;
        }
      }
      ...
      const children = depsMap.get(ref);
      if (children) {
        for (const childRef of children) {
          // Avoid circular dependencies - check if child is already in chain
          if (!chain.includes(childRef)) {
            queue.push({
              ref: childRef,
              chain: [...chain, ref],
            });
...
/**
 * Apply SrcFile dependency chains to a package list. The `chainsMap` is the
 * output of {@link buildDependencyChainsForward} (each chain already starts
 * with the lock file path and is followed by ancestor PURLs). Each package's
 * existing SrcFile property is replaced with the filtered chains (chains that
 * would include the package's own bom-ref are dropped). SrcFile paths are
 * then normalized relative to `projectRoot` (or the lock file's directory
 * when `projectRoot` is not provided).
 *
 * @param {Array} pkgList - Packages to annotate
 * @param {Map} chainsMap - Map of bom-ref to Array<Array<string>>
 * @param {string} lockFile - Path to the lock/metadata file
 * @param {string} [projectRoot] - Project root for path normalization
 */
function applySrcFileChains(pkgList, chainsMap, lockFile, projectRoot = null) {
  for (const pkg of pkgList) {
    const chains = chainsMap.get(pkg["bom-ref"]);
    let finalChains;
    if (chains && chains.length) {
      finalChains = chains.filter(
        (chain) => !chain.includes(pkg["bom-ref"]),
      );
    }
    // Packages absent from chainsMap (or that produced only self-referencing
    // chains) are treated as roots reachable directly from the lock file.
    if (!finalChains || !finalChains.length) {
      finalChains = [[lockFile]];
    }
    // Deep-copy the chains so duplicate pkgList entries (e.g. Swift trees
    // where the same package appears under multiple parents) don't share
    // arrays and accumulate normalization transformations.
    finalChains = finalChains.map((chain) => [...chain]);
    if (!pkg.properties) {
      pkg.properties = [];
    }
    const srcFileIdx = pkg.properties.findIndex((p) => p.name === "SrcFile");
    if (srcFileIdx >= 0) {
      pkg.properties[srcFileIdx].value = finalChains;
    } else {
      pkg.properties.push({ name: "SrcFile", value: finalChains });
    }
  }
  const baseDir = projectRoot || dirname(lockFile);
  normalizeSrcFilePaths(pkgList, baseDir);
}

/**
 * Apply dependency chains to package list with SrcFile property
 * Ensures consistent path normalization
 * 
 * @param {Array} pkgList - List of packages
 * @param {Map} chainsMap - Map of bom-ref to chains
 * @param {string} lockFileName - Lock file name
 * @param {string} lockFilePath - Full path to lock file
 * @param {string} projectRoot - Project root for normalization (optional)
 */
function applyDependencyChainsToPackages(pkgList, chainsMap, lockFileName, lockFilePath, projectRoot = null) {
  for (const pkg of pkgList) {
    const bomRef = pkg["bom-ref"];
    const chains = chainsMap.get(bomRef);
    
    if (!pkg.properties) {
      pkg.properties = [];
    }
    
    let srcFileProp = pkg.properties.find(p => p.name === "SrcFile");
    if (!srcFileProp) {
      srcFileProp = { name: "SrcFile", value: [] };
      pkg.properties.push(srcFileProp);
    }
    
    if (chains && chains.length > 0) {
      // Add lock file as first element of each chain
      srcFileProp.value = chains.map(chain => [lockFileName, ...chain]);
    } else {
      // No chains - this is a root dependency
      srcFileProp.value = [[lockFileName]];
    }
  }
  
  // Always normalize paths for consistency
  if (projectRoot) {
    normalizeSrcFilePaths(pkgList, projectRoot);
  } else if (lockFilePath) {
    // Fallback: normalize relative to lock file directory
    const baseDir = dirname(lockFilePath);
    normalizeSrcFilePaths(pkgList, baseDir);
  }
}

export function normalizeSrcFilePaths(pkgList, basePath) {
  if (!pkgList || !Array.isArray(pkgList) || !basePath) {
    return pkgList;
  }
  ...
  for (const pkg of pkgList) {
    ...
    const srcFileProp = pkg.properties.find(p => p.name === "SrcFile");
    if (!srcFileProp) {
      continue;
    }
    
    // Asar-style paths use "#/" to denote archive-internal segments and must
    // not be path-normalized (relative() would mangle them).
    const isAsarPath = (s) => typeof s === "string" && s.includes("#/");
    if (Array.isArray(srcFileProp.value)) {
      if (Array.isArray(srcFileProp.value[0])) {
        for (const chain of srcFileProp.value) {
          for (let i = 0; i < chain.length; i++) {
            const element = chain[i];
            if (i === 0 && typeof element === "string" && !isAsarPath(element)) {
              const rel = relative(basePath, element);
              chain[i] = rel || '.';
            }
          }
        }
      } else {
        srcFileProp.value = srcFileProp.value.map(v => {
          if (typeof v === "string" && !isAsarPath(v)) {
            const rel = relative(basePath, v);
            return rel || '.';
          }
          return v;
        });
      }
    } else if (typeof srcFileProp.value === "string" && !isAsarPath(srcFileProp.value)) {
      const rel = relative(basePath, srcFileProp.value);
      srcFileProp.value = rel || '.';
    }
  }
...
export async function parseGradleDep(
  ...
  gradleRootPath = "",
  buildFile = "",  // Path to build.gradle or build.gradle.kts
) {
  if (typeof rawOutput === "string") {
    ...
    const srcChain = []; // Track dependency chain: [config, dep1, dep2, dep3, ...]
    ...
    for (let rline of rawOutput.split("\n")) {
      ...
      // Detect configuration changes (e.g., "compileClasspath - Compile classpath")
      if (prevLineWasEmpty) {
        prevLineWasEmpty = false;
        const configMatch = rline.match(CONFIG_MATCHER);
        if (configMatch !== null) {
          // Reset chain to just the configuration name
          srcChain.splice(0, srcChain.length, configMatch[1]);
          // Don't continue - let it fall through to profileName detection below
        }
      }
      if (
      ...
      while ((match = depRegex.exec(rline))) {
        ...
        // Update chain based on dependency level
        srcChain.splice(Math.ceil(level));
        const depKey = `${group}:${name}@${version !== undefined ? version : ""}`;
        srcChain.push(depKey);
        ...
        if (version !== undefined || group === "project") {
          ...
          // Helper function to convert depKey to PURL
          const depKeyToPurl = (depKey) => {
            const parts = depKey.split(':');
            if (parts.length < 2) return depKey;
            const g = parts[0];
            const nameAndVer = parts.slice(1).join(':').split('@');
            const n = nameAndVer[0];
            const v = nameAndVer[1] || '';
            if (!v) return depKey; // Configuration name, keep as-is
            try {
              return new PackageURL("maven", g, n, v, { type: "jar" }, null).toString();
            } catch {
              return depKey;
            }
          };
          
          // Check if we've seen this dependency before
          if (depsMap.has(bomRef)) {
            const existingDep = depsMap.get(bomRef);
            // Convert chain elements to PURLs (except first element which is buildFile)
            const chain = [buildFile, ...srcChain.slice(0, -1).map(depKeyToPurl)];
            if (chain.length > 0) {
              const srcFileProp = getSrcFileProperty(existingDep);
              const chainKey = chain.join('#');
              const existingChainKeys = srcFileProp.value.map(c => Array.isArray(c) ? c.join('#') : c);
              if (!existingChainKeys.includes(chainKey)) {
                srcFileProp.value.push(chain);
              }
            }
          } else {
            // New dependency - create it
            let adep;
            if (gradleModules.has(name)) {
    ...
    // Convert Map to array - keep SrcFile as array of arrays (predictable format)
    const deps = [...depsMap.values()];
    
    // Normalize SrcFile paths to be relative to buildFile's directory
    if (buildFile) {
      const baseDir = dirname(buildFile);
      normalizeSrcFilePaths(deps, baseDir);
    }
 ...
 * @param {string} rawOutput Raw string output
 * @param {string} bazelFile Bazel build file path
 */
export function parseBazelSkyframe(rawOutput, bazelFile = null) {
  if (typeof rawOutput === "string") {
    const deps = [];
    const keys_cache = {};
    const tmpA = rawOutput.split("\n");
 ...
 * @param {string} rawOutput Raw output
 * @param {string} project Source project file
 */
export function parseKVDep(rawOutput, project = null) {
  if (typeof rawOutput === "string") {
    const deps = [];
    rawOutput.split("\n").forEach((l) => {
      l = l.replace("\r", "");
 ...
 * @param {string} piplockFile Path to Pipfile.lock
 */
export async function parsePiplockData(lockData, piplockFile = null, projectRoot = null) {
  const pkgList = [];
    ...
    .forEach((k) => {
      ...
      Object.keys(depBlock).forEach((p) => {
        ...
        if (Object.hasOwn(pkg, "version")) {
          ...
          const entry = { name: p, version: versionStr };
          if (piplockFile) {
            // Use array of arrays format for consistency
            entry.properties = [{
              name: "SrcFile",
              value: [[piplockFile]],
            }];
  ...
  // Normalize paths if projectRoot provided
  if (projectRoot && pkgList.length > 0) {
    normalizeSrcFilePaths(pkgList, projectRoot);
  }
 ...
 * @param {string} [projectRoot] Project root used to normalize SrcFile chain paths
 */
export async function parsePyLockData(lockData, lockFile, pyProjectFile, projectRoot = null) {
  let pkgList = [];
  const rootList = [];
  const dependenciesList = [];
  const depsMap = {};
  ...
  // Build SrcFile dependency chains from dependenciesList.
  // The first element of each chain is the lock file; subsequent elements are
  // PURLs of ancestor packages.
  if (pkgList?.length && dependenciesList?.length && lockFile) {
    const chainsMap = buildDependencyChainsForward(dependenciesList, lockFile);
    for (const pkg of pkgList) {
      const chains = chainsMap.get(pkg["bom-ref"]);
      if (!chains || !chains.length) {
        continue;
      }
      // Filter out any chains that contain the package itself (circular)
      ...
      const srcFileIdx = pkg.properties.findIndex(
        (p) => p.name === "SrcFile",
      );
      if (srcFileIdx >= 0) {
        pkg.properties[srcFileIdx].value = filteredChains;
...
async function parseReqData(reqFile, reqData = null, fetchDepsInfo = false) {
  ...
  for (const line of lines) {
    ...
    // Skip local file references (editable installs)
    if (/@\s+file:/.test(l)) {
      continue;
    }
    ...
    const properties = reqFile
      ? [
          {
            name: "SrcFile",
            value: [[reqFile]], // Array of arrays for consistency with other parsers
          },
        ]
      : [];
    const hashes = [];
  ...
  // Normalize SrcFile paths to be relative to reqFile directory
  if (reqFile) {
    const baseDir = dirname(reqFile);
    normalizeSrcFilePaths(pkgList, baseDir);
  }
...
export async function parseGoModGraph(
  ...
  // Build SrcFile chains for each package using BFS
  if (goModFile && pkgList.length > 0) {
    // Build a map of package bom-ref to package for quick lookup
    const pkgMap = new Map();
    for (const pkg of pkgList) {
      pkgMap.set(pkg["bom-ref"], pkg);
    }
    
    // Identify root dependencies (direct dependencies from go.mod)
    const rootRefs = new Set();
    const parentRef = goModPkgMap?.parentComponent?.["bom-ref"];
    if (parentRef) {
      const parentDeps = depsMap[parentRef];
      if (parentDeps) {
    ...
    // Fallback: if no parent component, treat all packages with no incoming edges as roots
    if (rootRefs.size === 0) {
      const hasIncoming = new Set();
      ...
      for (const ref of Object.keys(depsMap)) {
        if (!hasIncoming.has(ref) && ref !== parentRef) {
          rootRefs.add(ref);
        }
      }
    }
    
    // Build chains for each package using BFS
    const chainsMap = new Map(); // bomRef -> array of chains
    
    // Pre-compute chain strings for efficient duplicate detection
    const chainStringCache = new Map();
    
    // BFS to build all paths from roots to each descendant (optimized)
    for (const rootRef of rootRefs) {
      const queue = [{ ref: rootRef, chain: [] }]; // Start with empty chain for root
      let queueIndex = 0; // Use index instead of shift() for O(1) performance
      const visited = new Set();
      ...
      while (queueIndex < queue.length) {
        ...
        // Skip if we've already processed this ref with this exact chain
        const chainKey = `${ref}:${chain.join(",")}`;
        if (visited.has(chainKey)) {
          continue;
        }
        ...
        // Efficient duplicate detection using cached chain strings
        let chainStr = chainStringCache.get(chain);
        if (!chainStr) {
          chainStr = JSON.stringify(chain);
          chainStringCache.set(chain, chainStr);
        }
        
        let isDuplicate = false;
        for (const existingChain of existingChains) {
          let existingStr = chainStringCache.get(existingChain);
          if (!existingStr) {
            existingStr = JSON.stringify(existingChain);
            chainStringCache.set(existingChain, existingStr);
          }
          if (existingStr === chainStr) {
            isDuplicate = true;
            break;
        ...
        if (children) {
          for (const childRef of children) {
            ...
            if (!chain.includes(childRef)) {
              queue.push({
                ref: childRef,
                chain: [...chain, ref], // Add current ref as parent for children
              });
            }
          }
        }
    ...
    // Apply chains to packages
    const goModFileName = goModFile ? basename(goModFile) : "go.mod";
    for (const pkg of pkgList) {
      const bomRef = pkg["bom-ref"];
      const chains = chainsMap.get(bomRef);
      ...
      if (chains && chains.length > 0) {
        ...
        // Each chain starts with go.mod, followed by parent PURLs
        const srcFileChains = chains.map(chain => [goModFileName, ...chain]);
        pkg.properties.push({
          name: "SrcFile",
          value: srcFileChains,
        });
      } else {
        // No chains found - this is likely a direct dependency
        pkg.properties = pkg.properties || [];
        pkg.properties.push({
          name: "SrcFile",
          value: [[goModFileName]],
    ...
    if (projectRoot) {
      ...
      normalizeSrcFilePaths(pkgList, dirname(goModFile));
    }
  }
 ...
 * @param {string} gosumFile Path to go.sum file
 * @returns package list
 */
export async function parseGosumData(gosumData, gosumFile = null, projectRoot = null) {
  const pkgList = [];
  if (!gosumData) {
    return pkgList;
  }
  ...
  for (const l of pkgs) {
    ...
    if (m.indexOf("go.mod") > -1) {
      ...
      if (gosumFile) {
        ...
        component.properties.push({
          name: "SrcFile",
          value: [[gosumFile]], // Array of arrays for consistency
        });
      }
 ...
 * @param {string} [gopkgLockFile] Path to the source Gopkg.lock file for SrcFile property
 * @returns {Promise<Object[]>} List of Go package component objects
 */
export async function parseGopkgData(gopkgData, gopkgLockFile = null) {
  const pkgList = [];
  if (!gopkgData) {
    return pkgList;
  }
 ...
 * @param {string} [buildFile] Path to the source binary/build file for SrcFile property
 * @returns {Promise<Object[]>} List of Go package component objects
 */
export async function parseGoVersionData(buildInfoData, buildFile = null) {
  const pkgList = [];
  if (!buildInfoData) {
    return pkgList;
  }
 ...
 */
export async function parseGemfileLockData(gemLockData, lockFile, projectRoot = null) {
  let pkgList = [];
  const pkgnames = {};
  const dependenciesList = [];
  const dependenciesMap = {};
  ...
  // Build SrcFile dependency chains from dependenciesList
  if (pkgList?.length && dependenciesList?.length) {
    const gemfileLockPath = lockFile;
    
    // Use shared optimized BFS utility
    const chainsMap = buildDependencyChainsForward(dependenciesList, gemfileLockPath);
...
export async function parseCargoData(
  ...
  const addPackageToList = (packageList, newPackage, { simple }) => {
    ...
    if (!simple) {
      ...
      // Store metadata for chain building
      component._cargoMeta = {
      ...
      component.properties = [
        {
          name: "SrcFile",
          value: [[basename(cargoLockFile)]], // Will be updated with chains later
        },
      ];
  ...
  // Build dependency chains using the dependency information
  if (!simple && pkgList.length > 0) {
    // Build a map of package names to components
    const pkgMap = new Map();
    for (const pkg of pkgList) {
      pkgMap.set(pkg.name, pkg);
    }
    
    // Build dependency graph from Cargo.lock dependencies
    const depsMap = new Map();
    for (const pkg of pkgList) {
      if (pkg._cargoMeta?.dependencies) {
        const bomRef = pkg["bom-ref"];
        const deps = new Set();
        
        for (const dep of pkg._cargoMeta.dependencies) {
          // Dependencies can be in format "name version" or just "name"
          const depName = dep.includes(" ") ? dep.split(" ")[0].trim() : dep.trim();
          const depPkg = pkgMap.get(depName);
          if (depPkg) {
            deps.add(depPkg["bom-ref"]);
          }
        }
        
        if (deps.size > 0) {
          depsMap.set(bomRef, deps);
        }
      }
    }
    ...
    const chainsMap = new Map();
    const chainStringCache = new Map();
    const lockFileName = basename(cargoLockFile);
    ...
    for (const pkg of pkgList) {
      ...
      delete pkg._cargoMeta;
    }
    
    // Normalize paths relative to projectRoot
    if (projectRoot) {
      normalizeSrcFilePaths(pkgList, projectRoot);
    } else if (cargoLockFile) {
      const baseDir = dirname(cargoLockFile);
      normalizeSrcFilePaths(pkgList, baseDir);
    }
  }
 ...
 * @param {string} [conanLockFile] Path to the source conan.lock file for SrcFile property
 * @returns {{ pkgList: Object[], dependencies: Object, parentComponentDependencies: string[] }}
 */
export function parseConanLockData(conanLockData, conanLockFile = null, projectRoot = null) {
  const pkgList = [];
  const dependencies = {};
  ...
  if (pkgList.length && conanLockFile) {
    // Adapt the dependencies map to the {ref, dependsOn} shape expected by
    // buildDependencyChainsForward, and seed an implicit parent component
    // (parentComponentDependencies) so direct top-level requirements have a
    // chain rooted at the lock file rather than appearing as orphans.
    const adaptedDependenciesList = [];
    for (const ref of Object.keys(dependencies)) {
      adaptedDependenciesList.push({
        ref,
        dependsOn: dependencies[ref] || [],
      });
    }
    if (parentComponentDependencies.length) {
      const parentRef = `__conan_parent__:${conanLockFile}`;
      adaptedDependenciesList.push({
        ref: parentRef,
        dependsOn: parentComponentDependencies,
      });
    }
    const chainsMap = adaptedDependenciesList.length
      ? buildDependencyChainsForward(adaptedDependenciesList, conanLockFile)
      : new Map();
    // Strip the synthetic parent ref out of every chain so it never leaks
    // into SrcFile output.
    const cleanedChainsMap = new Map();
    const parentRefPrefix = `__conan_parent__:`;
    for (const [k, chains] of chainsMap) {
      cleanedChainsMap.set(
        k,
        chains.map((chain) =>
          chain.filter((seg) => !seg.startsWith(parentRefPrefix)),
        ),
      );
    }
    applySrcFileChains(pkgList, cleanedChainsMap, conanLockFile, projectRoot);
  }
  return { pkgList, dependencies, parentComponentDependencies };
}
 ...
 * @param {string} [conanFile] Path to the source conanfile.txt for SrcFile property
 * @returns {Object[]} Array of package objects with purl, name, version, and scope
 */
export function parseConanData(conanData, conanFile = null) {
  const pkgList = [];
  if (!conanData) {
    return pkgList;
  }
 ...
 */
export function parseCsProjAssetsData(csProjData, assetsJsonFile, projectRoot = null) {
  ...
  if (pkgList.length && dependenciesList.length && assetsJsonFile) {
    const chainsMap = buildDependencyChainsForward(
      dependenciesList,
      assetsJsonFile,
    );
    applySrcFileChains(pkgList, chainsMap, assetsJsonFile, projectRoot);
  }
  return {
    pkgList,
    dependenciesList,
 ...
 */
export function parseCsPkgLockData(csLockData, pkgLockFile, projectRoot = null) {
  const pkgList = [];
  const dependenciesList = [];
  const rootList = [];
  let pkg = null;
  ...
  if (pkgList.length && dependenciesList.length && pkgLockFile) {
    const chainsMap = buildDependencyChainsForward(
      dependenciesList,
      pkgLockFile,
    );
    applySrcFileChains(pkgList, chainsMap, pkgLockFile, projectRoot);
  }
  return {
    pkgList,
    dependenciesList,
 ...
 * @returns {{ pkgList: Object[], dependenciesList: Object[] }}
 */
export function parsePaketLockData(paketLockData, pkgLockFile, projectRoot = null) {
  const pkgList = [];
  const dependenciesList = [];
  const dependenciesMap = {};
  const pkgNameVersionMap = {};
 ...
 * @param {string} [projectRoot] Project root used to normalize SrcFile chain paths
 */
export function parseComposerLock(pkgLockFile, rootRequires, projectRoot = null) {
  const pkgList = [];
  const dependenciesList = [];
  const dependenciesMap = {};
  ...
  // Build SrcFile dependency chains: replace the single-string SrcFile with
  // Array<Array<string>> chains rooted at the lock file path, with ancestor
  // PURLs trailing. Multiple resolution paths are stored as separate chains.
  if (pkgList.length && dependenciesList.length && pkgLockFile) {
    const chainsMap = buildDependencyChainsForward(
      dependenciesList,
      pkgLockFile,
...
export function parseSbtTree(sbtTreeFile) {
  ...
  let stack = [];
  let first_purl = "";
  const srcChain = []; // Track dependency chain: [sbtTreeFile, dep1, dep2, ...]
  ...
  tmpA.forEach((l) => {
    ...
    // Update srcChain based on level changes
    if (level === 0) {
      // Direct dependency - chain is [sbtTreeFile, dep]
      srcChain.length = 0;
      srcChain.push(sbtTreeFile);
      srcChain.push(`${group}:${name}:${version}`);
    } else if (level > last_level) {
      // Going deeper - add current dep to chain
      srcChain.push(`${group}:${name}:${version}`);
    } else if (level <= last_level) {
      // Same level or going back up - trim chain to current level
      // Keep sbtTreeFile + (level) deps
      const targetLength = 1 + level + 1;
      srcChain.length = targetLength;
      srcChain[srcChain.length - 1] = `${group}:${name}:${version}`;
    }
    
    // Filter duplicates
    if (!keys_cache[purlString]) {
      // Store chain excluding the package itself (slice(0, -1) to remove last element)
      const chainWithoutSelf = srcChain.slice(0, -1);
      
      // Convert chain elements to PURLs (except first element which is sbtTreeFile)
      const purlChain = chainWithoutSelf.map((item, index) => {
        if (index === 0) return item; // Keep file path as-is
        const parts = item.split(':');
        if (parts.length === 3) {
          try {
            return new PackageURL("maven", parts[0], parts[1], parts[2], { type: "jar" }, null).toString();
          } catch {
            return item;
          }
        }
      ...
      const properties = [{
        name: "SrcFile",
        value: [purlChain], // Store current resolution path as array of arrays
      }];
      ...
      depsMap.set(purlString, adep);
      keys_cache[purlString] = true;
    } else {
      // Package already exists - add this chain as another resolution path
      const existingPkg = depsMap.get(purlString);
      if (existingPkg) {
        const srcFileProp = getSrcFileProperty(existingPkg);
        // Store chain excluding the package itself
        const chainWithoutSelf = srcChain.slice(0, -1);
        
        // Convert chain elements to PURLs
        const purlChain = chainWithoutSelf.map((item, index) => {
          if (index === 0) return item; // Keep file path as-is
          const parts = item.split(':');
          if (parts.length === 3) {
        ...
        // Check if this chain is unique
        const chainStr = JSON.stringify(purlChain);
        const isDuplicate = srcFileProp.value.some(
          existingChain => JSON.stringify(existingChain) === chainStr
        );
        if (!isDuplicate) {
          srcFileProp.value.push(purlChain);
        }
      }
    }
  ...
  // Convert Map to array
  const pkgList = [...depsMap.values()];
  
  // Normalize SrcFile paths to be relative to sbtTreeFile directory
  const baseDir = dirname(sbtTreeFile);
  normalizeSrcFilePaths(pkgList, baseDir);
 ...
 * @param {string} pkgFile Package.swift file
 */
export function parseSwiftJsonTree(rawOutput, pkgFile, projectRoot = null) {
  if (!rawOutput) {
    return {};
  }
  const pkgList = [];
  ...
  if (pkgList.length && dependenciesList.length && pkgFile) {
    const chainsMap = buildDependencyChainsForward(dependenciesList, pkgFile);
    applySrcFileChains(pkgList, chainsMap, pkgFile, projectRoot);
  }
  return {
    rootList,
    pkgList,
...
export async function extractJarArchive(jarFile, tempDir, jarNSMapping = {}) {
  ...
  if (jarFiles?.length) {
    for (const jf of jarFiles) {
      ...
      // Ignore test jars and JCE policy files
      if (
        jarname.endsWith("-tests.jar") ||
        jarname.endsWith("-test-sources.jar") ||
        jarname === "local_policy.jar" ||
        jarname === "US_export_policy.jar"
      ) {
        if (DEBUG_MODE) {
          console.log(`Skipping ${jarname}`);
        }
        continue;
      }
...
export function getMavenCommand(srcPath, rootPath) {
  ...
  if (isWrapperFound) {
    ...
    if (!result.error && !result.status) {
      ...
      thoughtLog(`**MAVEN**: Using Maven Wrapper (mvnw) found in the project.`);
    } else {
      if (DEBUG_MODE) {
        console.log(
...
function flattenDeps(dependenciesMap, pkgList, reqOrSetupFile, t, parentChain = []) {
  const tRef = `pkg:pypi/${t.name.replace(/_/g, "-").toLowerCase()}@${
    t.version
  }`;
  const dependsOn = [];
  
  // Build current chain: [reqOrSetupFile, parent1, parent2, ..., current]
  const currentChain = reqOrSetupFile ? [reqOrSetupFile, ...parentChain, `${t.name}:${t.version}`] : [];
  ...
  for (const d of t.dependencies) {
    ...
    if (reqOrSetupFile) {
      // Add dependency chain - includes all parents leading to this dependency
      const depChain = [...currentChain, `${d.name}:${d.version}`];
      apkg.properties = [
        {
          name: "SrcFile",
          value: [depChain], // Array of arrays for consistency
        },
      ];
      apkg.evidence = {
        identity: {
    ...
    pkgList.push(apkg);
    // Recurse and flatten - pass the chain for further nesting
    if (d.dependencies && d.dependencies.length) {
      flattenDeps(dependenciesMap, pkgList, reqOrSetupFile, d, [...parentChain, `${t.name}:${t.version}`]);
    }
  }
...
/**
 * Run pigar in an isolated temporary virtual environment to generate requirements.txt
 * This avoids polluting the user's Python environment
 *
 * @param {string} setupDir Directory containing setup.py
 * @param {string} tempVenvDir Temporary venv directory to use
 */
export async function runPigarInVenv(setupDir, tempVenvDir) {
  try {
    ...
    // Create a temporary venv if it doesn't exist yet (check for python executable, not just dir)
    if (!safeExistsSync(venvPython)) {
      const venvResult = safeSpawnSync(PYTHON_CMD, ["-m", "venv", tempVenvDir], {
        shell: isWin,
        encoding: "utf-8",
      });
      if (venvResult.status !== 0 || venvResult.error) {
        return {
          success: false,
          error: "Failed to create venv",
          details: venvResult.stderr || venvResult.error?.message,
        };
      }
    }

    // Verify the python executable exists after venv creation
    if (!safeExistsSync(venvPython)) {
      return {
        success: false,
        error: "Venv python not found",
        details: `Expected at: ${venvPython}`,
      };
    }

    // Install latest pigar and setuptools in the venv
    const pipInstallResult = safeSpawnSync(
      venvPython,
      ["-m", "pip", "install", "-q", "pigar", "setuptools"],
      {
        encoding: "utf-8",
        shell: isWin,
        timeout: 60000,
    ...
    if (pipInstallResult.status !== 0 || pipInstallResult.error) {
      return {
        success: false,
        error: "Failed to install pigar and setuptools",
        details: pipInstallResult.stderr || pipInstallResult.error?.message,
      };
    }

    // Run pigar to generate requirements file
    // Using pigar gen: pigar gen -f <output-file> <directory>
    // Provide stdin input to handle prompts
    const pigarResult = safeSpawnSync(
      venvPython,
      ["-m", "pigar", "gen", "-f", "extracted-requirements.txt", "."],
      {
        cwd: setupDir,
        encoding: "utf-8",
        shell: isWin,
        timeout: 30000,
        input: "y\nrequests\n", // Confirm PyPI search and select package
      }
    );

    if (pigarResult.status !== 0 || pigarResult.error) {
      return {
        success: false,
        error: "Pigar execution failed",
        details: pigarResult.stdout || pigarResult.stderr || pigarResult.error?.message,
      };
    }

    // Filter out pigar itself from the generated requirements
    // (pigar was installed in the venv but shouldn't be in project requirements)
    const { readFileSync, writeFileSync } = await import("node:fs");
    const reqFilePath = join(setupDir, "extracted-requirements.txt");
    if (safeExistsSync(reqFilePath)) {
      const content = readFileSync(reqFilePath, "utf-8");
      const filtered = content
        .split("\n")
        .filter(line => !line.toLowerCase().includes("pigar"))
        .join("\n");
      writeFileSync(reqFilePath, filtered, "utf-8");
    }
...
export async function getPipFrozenTree(
    ...
    for (const t of tree) {
      ...
      const apkg = {
        ...
        properties: [
          {
            name: "SrcFile",
            value: [[reqOrSetupFile, `${name}:${version}`]], // Array of arrays: [file, package]
          },
        ],
      };
      if (scope !== "excluded") {
  ...
  // Normalize SrcFile paths to be relative to basePath
  normalizeSrcFilePaths(pkgList, basePath);
...
export async function addEvidenceForImports(
  ...
  for (const pkg of pkgList) {
    ...
    // Check if this package has an alias (e.g., pdfjs-dist-v4_9 for pdfjs-dist)
    let packageAlias = null;
    if (pkg.properties) {
      for (const prop of pkg.properties) {
        if (prop.name === "cdx:npm:package:alias") {
          packageAlias = prop.value;
          break;
        }
      }
    }
    // Build list of aliases to check
    // If package has an alias, ONLY match the alias (not the actual name)
    // This prevents aliased packages from matching regular imports
...
/**
 * Add occurrences from Java import analysis to package list
 * 
 * @param {Array} pkgList List of packages
 * @param {Object} javaImports Map of package prefixes to Sets of occurrence objects
 * @returns {Array} Updated package list with occurrences
 */
export function addJavaOccurrences(pkgList, javaImports) {
  const impPkgs = Object.keys(javaImports);
  
  for (const pkg of pkgList) {
    const { group, name } = pkg;
    
    // Build possible package prefixes to match against imports
    // For Maven: group is like "org.springframework", name is like "spring-core"
    // Java imports look like: org.springframework.core.* or org.springframework.boot.*
    const searchKeys = [];
    
    if (group) {
      // Direct group match (e.g., org.springframework)
      searchKeys.push(group);
      
      // Group + name variations (e.g., org.springframework.boot for spring-boot-starter)
      // Handle artifacts like spring-boot-starter -> spring.boot
      const nameParts = name.split('-');
      if (nameParts.length > 1) {
        searchKeys.push(`${group}.${nameParts[0]}`);
        if (nameParts.length > 2) {
          searchKeys.push(`${group}.${nameParts[0]}.${nameParts[1]}`);
        }
      }
    }
    ...
    for (const searchKey of searchKeys) {
      const matchingKeys = impPkgs.filter(key => 
      ...
      if (matchingKeys.length > 0) {
        ...
        for (const key of matchingKeys) {
          const fileNames = javaImports[key]; // This is now a Set of strings
          for (const fileName of fileNames) {
            if (fileName && !seenLocations.has(fileName)) {
              seenLocations.add(fileName);
              
              if (!pkg.evidence) {
                pkg.evidence = {};
              }
              if (!pkg.evidence.occurrences) {
        ...
        // Mark as required if we found imports
    ...
    // Also check for reverse case where import package is part of group
    // E.g., com.google.common.* imports match com.google.guava artifact
    if (group && !pkg.evidence?.occurrences) {
      const groupPrefix = group.split('.').slice(0, 2).join('.'); // Get first 2 segments
      const matchingKeys = impPkgs.filter(key => 
      ...
      if (matchingKeys.length > 0) {
        ...
        for (const key of matchingKeys) {
          const fileNames = javaImports[key];
          for (const fileName of fileNames) {
            if (fileName && !seenLocations.has(fileName)) {
              seenLocations.add(fileName);
...
/**
 * Add occurrences from Ruby require analysis to package list
 * 
 * @param {Array} pkgList List of packages
 * @param {Object} rubyImports Map of gem names to Sets of occurrence objects
 * @returns {Array} Updated package list with occurrences
 */
export function addRubyOccurrences(pkgList, rubyImports) {
  const impGems = Object.keys(rubyImports);
  
  for (const pkg of pkgList) {
    const { name } = pkg;
    
    // Ruby gem names can have dashes or underscores
    // Match both forms: 'active-support' and 'active_support'
    const nameVariants = [
      name,
      name.replace(/-/g, '_'),
      name.replace(/_/g, '-'),
    ...
    for (const variant of nameVariants) {
      if (impGems.includes(variant)) {
        const fileNames = rubyImports[variant]; // This is now a Set of strings
        const seenLocations = new Set();
        ...
        // Mark as required if we found requires
        if (seenLocations.size > 0) {
          pkg.scope = "required";
        }
...
/**
 * Add occurrences from Clojure require/import analysis to package list
 * 
 * @param {Array} pkgList List of packages
 * @param {Object} clojureImports Map of package names to Sets of file paths
 * @returns {Array} Updated package list with occurrences
 */
export function addClojureOccurrences(pkgList, clojureImports) {
  const impPkgs = Object.keys(clojureImports);
  
  for (const pkg of pkgList) {
    const { group, name } = pkg;
    
    // Clojure packages typically map to group/name
    // e.g., com.github.user/library
    const fullName = group ? `${group}.${name}` : name;
    
    // Try exact match first
    let matchingKey = impPkgs.find(key => key === fullName);
    
    // If no exact match, try with dashes converted to underscores
    // Clojure allows both forms
    if (!matchingKey) {
      const nameWithUnderscore = fullName.replace(/-/g, '_');
      matchingKey = impPkgs.find(key => key === nameWithUnderscore);
    }
    
    // Try matching just the artifact name for simple cases
    if (!matchingKey) {
      matchingKey = impPkgs.find(key => 
        key === name || 
        key === name.replace(/-/g, '_') ||
        key.endsWith('.' + name) ||
        key.endsWith('.' + name.replace(/-/g, '_'))
      );
    }
    
    if (matchingKey) {
      const fileNames = clojureImports[matchingKey];
      const seenLocations = new Set();
