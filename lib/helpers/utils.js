...
export function parseMavenTree(rawOutput, pomFile) {
  ...
  let first_ref;
  const stack = [];
  const srcChain = []; // Track dependency chain: [pomFile, scope, dep1, dep2, ...]
  ...
  tmpA.forEach((l) => {
    ...
    if (tmpline?.length) {
      ...
      if (pkgArr && pkgArr.length > 2) {
        ...
        if (level === 0) {
          // Direct dependency - chain is [pomFile, scope, dep]
          srcChain.length = 0;
          if (pomFile) {
            srcChain.push(pomFile);
          }
          srcChain.push(componentScope);
          srcChain.push(`${pkgArr[0]}:${pkgArr[1]}:${versionStr}`);
        } else if (level > last_level) {
          // Going deeper - add current dep to chain
          srcChain.push(`${pkgArr[0]}:${pkgArr[1]}:${versionStr}`);
        } else if (level <= last_level) {
          // Same level or going back up - trim chain to current level
          // Keep pomFile + scope + (level) deps
          const targetLength = (pomFile ? 1 : 0) + 1 + level + 1;
          srcChain.length = targetLength;
          srcChain[srcChain.length - 1] = `${pkgArr[0]}:${pkgArr[1]}:${versionStr}`;
        }
        ...
        if (!keys_cache[key]) {
          ...
          // Add SrcFile as array of arrays with full chain
          if (pomFile) {
            properties.push({
              name: "SrcFile",
              value: [[...srcChain]], // Store current resolution path
            });
          }
  ...
  // Normalize SrcFile paths to be relative to pomFile directory
  if (pomFile) {
    const baseDir = dirname(pomFile);
      ...
      while ((match = depRegex.exec(rline))) {
        ...
        if (version !== undefined || group === "project") {
          ...
          if (depsMap.has(bomRef)) {
            ...
            const chain = [buildFile, ...srcChain.slice(0, -1)];
            if (chain.length > 0) {
 ...
 */
export async function parsePiplockData(lockData, piplockFile = null) {
  const pkgList = [];
 ...
 */
export async function parseGosumData(gosumData, gosumFile = null) {
  const pkgList = [];
  if (!gosumData) {
    return pkgList;
  }
...
export function parseSbtTree(sbtTreeFile) {
  ...
  tmpA.forEach((l) => {
    ...
    if (!keys_cache[purlString]) {
      const properties = [{
        name: "SrcFile",
        value: [[...srcChain]], // Store current resolution path as array of arrays
      }];
