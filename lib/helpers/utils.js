 ...
 */
export function runPigarInVenv(setupDir, tempVenvDir) {
  try {
    ...
    if (!safeExistsSync(venvPython)) {
      ...
      if (venvResult.status !== 0 || venvResult.error) {
        if (DEBUG_MODE) {
          console.log("Failed to create venv for pigar:", venvResult.stderr);
        }
        return;
      }
    }
    ...
    if (!safeExistsSync(venvPython)) {
      if (DEBUG_MODE) {
        console.log("Venv python not found at:", venvPython);
      }
      return;
    }

    // Install pigar in the venv
    const pipInstallResult = safeSpawnSync(
      venvPython,
      ["-m", "pip", "install", "-q", "pigar"],
      {
        encoding: "utf-8",
        shell: isWin,
        timeout: 60000, // 60 second timeout for network install
      }
    );
    
    if (pipInstallResult.status !== 0 || pipInstallResult.error) {
      if (DEBUG_MODE) {
        console.log("Failed to install pigar in venv:", pipInstallResult.stderr);
      }
      return;
    }

    // Run pigar to generate extracted-requirements.txt
    const pigarResult = safeSpawnSync(
      venvPython,
      ["-m", "pigar", "-y", "-p", "extracted-requirements.txt"],
      {
        cwd: setupDir,
        encoding: "utf-8",
        shell: isWin,
        timeout: 30000, // 30 second timeout for pigar execution
      }
    );

    if (DEBUG_MODE && (pigarResult.status !== 0 || pigarResult.error)) {
      console.log("Pigar execution completed with status:", pigarResult.status);
    }
  } catch (ex) {
    // Silently ignore pigar failures - it's an optional enhancement
    if (DEBUG_MODE) {
      console.log("Exception running pigar:", ex.message);
    }
  }
}
