...
export function normalizeSrcFilePaths(pkgList, basePath) {
  ...
  for (const pkg of pkgList) {
    ...
    } else if (typeof srcFileProp.value === "string") {
...
export async function addEvidenceForImports(
  ...
  for (const pkg of pkgList) {
    ...
    for (const alias of aliases) {
      // For aliased packages, use stricter matching to avoid false matches
      // We want to match: 'pdfjs-dist-v4_9' or 'pdfjs-dist-v4_9/web/pdf_viewer.mjs'
      // But NOT: 'pdfjs-dist-v4_9-something-else'
      // Use same logic as regular packages but only for the alias
        ...
        for (const subevidence of all_includes) {
          ...
          for (const evidence of evidences) {
            if (evidence && Object.keys(evidence).length && evidence.fileName) {
              ...
              // Only add location (file path), not line numbers
              const location = evidence.fileName;
              // Deduplicate - don't add the same location twice
              if (!pkg.evidence.occurrences.some(occ => occ.location === location)) {
