...
/**
 * Find all Clojure imports/requires in source files
 * 
 * @param {string} src Source directory
 * @returns {Object} Map of package names to Sets of file paths
 */
export const findClojureImports = (src) => {
  const allImports = {}; // {packageName: Set<string>}
  
  try {
    const findClojureFiles = (dir, files = []) => {
      try {
        const entries = readdirSync(dir, { withFileTypes: true });
        for (const entry of entries) {
          const fullPath = join(dir, entry.name);
          if (entry.isDirectory()) {
            // Skip common non-source directories
            if (!['target', '.cpcache', 'node_modules', 'resources', '.git'].includes(entry.name)) {
              findClojureFiles(fullPath, files);
            }
          } else if (entry.isFile() && /\.(clj|cljs|cljc)$/.test(entry.name)) {
            files.push(fullPath);
          }
        }
      } catch (_err) {
    ...
    const clojureFiles = findClojureFiles(src);
    
    // Pattern 1: (require '[package.name :as alias])
    const requirePattern = /\(require\s+'\[([a-zA-Z0-9._-]+)/g;
    
    // Pattern 2: [package.name ...] in (:require ...) forms - finds ALL require statements
    const nsRequirePattern = /\[([a-zA-Z0-9._-]+(?:\.[a-zA-Z0-9._-]+)*)\s+(?::as|:refer)/g;
    
    // Pattern 3: (:import [package.Class]) for Java interop
    const importPattern = /\[([a-zA-Z][a-zA-Z0-9.]+)\s*\]/g;
    
    // Pattern 4: (import '[package.Class])
    const importQuotePattern = /\(import\s+'\[?([a-zA-Z][a-zA-Z0-9.]+)/g;
    
    for (const file of clojureFiles) {
      try {
        const content = readFileSync(file, 'utf-8');
        const fileRelativeLoc = relative(src, file);
        
        // Find all require statements - Pattern 1
        let match;
        const requireClone = new RegExp(requirePattern.source, requirePattern.flags);
        while ((match = requireClone.exec(content)) !== null) {
          const pkgName = match[1];
          // Skip clojure.* core libs and java.* stdlib
          if (!pkgName.startsWith('clojure.') && !pkgName.startsWith('java.') && !pkgName.startsWith('javax.')) {
            if (!allImports[pkgName]) {
              allImports[pkgName] = new Set();
            }
            allImports[pkgName].add(fileRelativeLoc);
          }
        }
        
        // Find all ns-form requires - Pattern 2
        // This handles (:require [pkg.name :as alias] [other.pkg :refer [fn]])
        const nsRequireClone = new RegExp(nsRequirePattern.source, nsRequirePattern.flags);
        while ((match = nsRequireClone.exec(content)) !== null) {
          const pkgName = match[1];
          // Skip clojure.* core libs and java.* stdlib
          if (!pkgName.startsWith('clojure.') && !pkgName.startsWith('java.') && !pkgName.startsWith('javax.')) {
            if (!allImports[pkgName]) {
        ...
        // Find Java import statements in (:import ...) blocks - Pattern 3
        // First, extract all (:import ...) blocks
        const importBlockPattern = /\(:import[^)]*\)/gs;
        const importBlocks = content.match(importBlockPattern) || [];
        for (const block of importBlocks) {
          const importClone = new RegExp(importPattern.source, importPattern.flags);
          while ((match = importClone.exec(block)) !== null) {
            const className = match[1];
            // Skip java.* and javax.* standard library
            if (!className.startsWith('java.') && !className.startsWith('javax.')) {
              // Extract package prefix (first two parts)
              const parts = className.split('.');
              if (parts.length >= 2) {
                const pkgKey = `${parts[0]}.${parts[1]}`;
                if (!allImports[pkgKey]) {
                  allImports[pkgKey] = new Set();
                }
                allImports[pkgKey].add(fileRelativeLoc);
              }
            }
          }
        }
        
        // Find standalone (import ...) statements - Pattern 4
        const importQuoteClone = new RegExp(importQuotePattern.source, importQuotePattern.flags);
        while ((match = importQuoteClone.exec(content)) !== null) {
          const className = match[1];
          // Skip java.* and javax.* standard library
          if (!className.startsWith('java.') && !className.startsWith('javax.')) {
            // Extract package prefix (first two parts)
