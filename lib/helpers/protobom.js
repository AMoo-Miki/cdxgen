...
/**
 * Stringify any properties[].value that is not already a string so the proto
 * schema (which mandates string) does not throw. cdxgen permits SrcFile and
 * similar properties to carry an array (chain) value, but proto encoding needs
 * a serialized form.
 */
const stringifyArrayPropertyValues = (subject, seen = new Set()) => {
  ...
  if (Array.isArray(subject)) {
    for (const item of subject) {
      stringifyArrayPropertyValues(item, seen);
    }
    return;
  }
  if (Array.isArray(subject.properties)) {
    for (const property of subject.properties) {
      if (
        ...
        try {
          property.value = JSON.stringify(property.value);
        } catch (_e) {
  ...
  for (const value of Object.values(subject)) {
    if (value && typeof value === "object") {
      stringifyArrayPropertyValues(value, seen);
    }
  }
};
...
const normalizeObjectWrappedListsForProto = (bomJson) => {
  ...
  stringifyArrayPropertyValues(normalizedBomJson);
