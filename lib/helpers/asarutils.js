...
function toArchiveVirtualPath(extractedDir, archivePath, candidatePath) {
  ...
  // Already an asar virtual path (from a nested archive): leave as is
  ...
  // Relative path: only treat as a file path candidate when it actually exists
  // under the extracted directory. This avoids rewriting non-path scalar
  // values like "true"/"false"/version numbers.
  const candidateAbs = resolve(normalizedExtractedDir, candidatePath);
  if (!isPathWithin(normalizedExtractedDir, candidateAbs)) {
    return candidatePath;
  }
  if (!safeExistsSync(candidateAbs)) {
    return candidatePath;
  }
  const relativePath = relative(normalizedExtractedDir, candidateAbs);
  return `${archivePath}#/${relativePath.replaceAll("\\", "/")}`;
}

function rewriteCandidatePathValue(value, extractedDir, archivePath) {
  if (typeof value === "string") {
    return toArchiveVirtualPath(extractedDir, archivePath, value);
  }
  if (Array.isArray(value)) {
    return value.map((entry) =>
      rewriteCandidatePathValue(entry, extractedDir, archivePath),
    );
  }
  return value;
}
...
export function rewriteExtractedArchivePaths(
  ...
  if (subject.properties?.length) {
    subject.properties.forEach((property) => {
      ...
      property.value = rewriteCandidatePathValue(
        property.value,
        extractedDir,
        archivePath,
      );
